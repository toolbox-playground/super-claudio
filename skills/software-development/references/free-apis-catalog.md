...
| OpenRouter | Access to 100+ free AI models via one API; notable free models (as of Aug 2026): **NVIDIA Nemotron 3 Ultra** (550B MoE, 1M context, `nvidia/nemotron-3-ultra-550b-a55b:free`), **OpenAI GPT-OSS** (20B Apache 2.0 open-weight, `openai/gpt-oss-20b:free`), and Google's **Gemma 4** (26B/31B, 262K context); 25-29 free models total; 50 req/day free, 1,000/day after $10 purchase. **Note:** Stripe agreed to acquire OpenRouter for $7B+ (announced Aug 16, 2026); OpenRouter says product, name, and free tier are unchanged | openrouter.ai | Free key |
...
| Hugging Face Inference API | Serverless inference across 100,000+ open models (text, image, audio, embeddings) — broadest model selection of any free provider; free tier rate-limited (~few hundred req/hour, models under ~10B params, cold starts on unpopular models); PRO ($9/mo) raises limits substantially | huggingface.co/docs/inference-providers | Free account (HF token) |
