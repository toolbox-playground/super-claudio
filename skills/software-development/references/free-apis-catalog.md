...
| **Hugging Face Inference Providers** | Unified router API for serverless inference across open models (Llama, Qwen, Whisper, Stable Diffusion, and more) hosted by multiple providers; free accounts get a small monthly inference credit for prototyping (PRO tier raises the credit); full Hub access (2M+ models, 500k+ datasets) always free, no key needed for browsing | huggingface.co/docs/inference-providers | Free account (key for inference) |
