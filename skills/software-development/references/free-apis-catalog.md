...
| NVIDIA NIM | Free access to 100+ open-weight AI models (Llama, Mistral, DeepSeek-R1, Nemotron, and more) hosted on NVIDIA infrastructure; OpenAI-compatible Chat Completions endpoint with streaming and tool calling; no credit card required; ~40 RPM on free tier, no daily token cap | build.nvidia.com | Free key (phone verification) |
