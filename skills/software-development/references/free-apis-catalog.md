...
| Open Notify | ISS current location and astronauts in space; **ISS Pass Times endpoint was removed in 2026** — only `iss-now.json` and astronaut endpoints remain live | open-notify.org | None |
...
| CoinGecko | Crypto prices and data; pure keyless access is now heavily rate-limited — register a free Demo API key for reliable use (100 calls/min, 10,000 credits/month, ~30 of 70+ endpoints) | coingecko.com/api | None (rate limited); free Demo key recommended |
...
| API | What | URL | Auth |
|-----|------|-----|------|
| **TheSportsDB** | Free sports database — teams, players, events, live scores, standings, artwork across football, basketball, and more | thesportsdb.com/free_sports_api | Shared free test key `123` (30 req/min, capped results); optional paid key for higher limits/V2 |
