...
| **GitHub Models** | **⚠️ Fully retired July 30, 2026** — GitHub shut down the playground, model catalog, inference API, and BYOK for all customers (including those with prior active usage), after closing new signups on June 16, 2026. GitHub now points users to Microsoft Foundry or GitHub Copilot for model access. Use Groq, OpenRouter, Cerebras, or Cloudflare Workers AI instead. | github.com/features/models (now redirects to Copilot) | Retired |
...
| **LiteLLM Model Pricing & Context Window Data** | Daily-updated JSON feed of input/output token pricing, context-window sizes, and capability flags (vision, function calling, reasoning) for 1,000+ models across 100+ providers (OpenAI, Anthropic, Google, Bedrock, etc.); maintained by the LiteLLM project (59k+ GitHub stars, updated daily); plain raw JSON file, no key or wrapper needed | github.com/BerriAI/litellm (raw file: `model_prices_and_context_window.json`) | None |
...
- **Awesome Free LLM APIs:** github.com/open-free-llm-api/awesome-freellm-apis — 134+ free LLM APIs and AI API keys from 40+ providers (Gemini, NVIDIA NIM, Groq, OpenRouter, and more); one-click setup snippets for Claude Code, Cursor, and Codex; 3,100+ GitHub stars, actively updated
