...
| API | What | URL | Auth |
|-----|------|-----|------|
| Google Gemini API | LLM inference — Gemini 3 Flash / Flash-Lite family free in AI Studio; **Pro-series models (Gemini 2.5 Pro, 3 Pro, 3.1 Pro) removed from the free tier on April 1, 2026** — Pro is now paid-only, Flash/Flash-Lite remain free; 1M token context, multimodal | ai.google.dev | Free key (generous limits) |
| Groq | Ultra-fast LLM inference (LPU hardware); Llama 4 Scout, Llama 4 Maverick, Kimi K2 (1T MoE), Qwen3 235B, Gemma, Mistral, DeepSeek R1 and more; no credit card required; free tier: 30 RPM, ~14,400 req/day (varies by model) | groq.com | Free key |
| OpenRouter | Access to 20+ free AI models via one API; notable free models (Sep 2026): **NVIDIA Nemotron 3 Ultra** (550B MoE, 1M context, `nvidia/nemotron-3-ultra-550b-a55b:free`) still tops usage rankings alongside newer entrants; 50 req/day free, 1,000/day after $10 lifetime purchase | openrouter.ai | Free key |
| **GitHub Models** | **⚠️ Retired (July 30, 2026)** — GitHub fully shut down the free Models playground, catalog, inference API, and BYOK access for all customers, including prior users; no replacement offered. Use Groq, Cerebras, OpenRouter, or Cloudflare Workers AI instead. | github.com/features/models | N/A — discontinued |
