...
| **Tons of Skills** | tonsofskills.com | 471 plugins, 3,069 skills, 347 agents for Claude Code (Sept 2026, up from 200 agents in July); `ccpi` CLI package manager; catalog updated daily from GitHub; most comprehensive single-site collection |
| **agentskill.sh** | agentskill.sh | Cross-agent skill marketplace (Claude Code, Cursor, Copilot, Codex, Windsurf, Zed, 20+ tools); 275,000+ skills (grew from ~100K at March 2026 launch); `/learn` command lets an agent auto-search and self-install a skill mid-task; every skill server-side scanned across 12 threat categories, with client-side re-verification on install |
