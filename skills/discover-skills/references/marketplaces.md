...
| **wshobson/agents** | github.com/wshobson/agents | Multi-harness marketplace: 92 plugins, 202 agents, 181 skills, 105 commands; native support for Claude Code, Codex CLI, Cursor, OpenCode, Gemini CLI, and GitHub Copilot — each harness gets idiomatic artifacts from one Markdown source |
| **hesreallyhim/awesome-claude-code** | github.com/hesreallyhim/awesome-claude-code | Hand-curated (not auto-scraped) list of slash-commands, CLAUDE.md files, hooks, agent orchestrators, and tooling for Claude Code; 52.8k+ GitHub stars — one of the most-starred Claude Code resource lists |
...
| **Cross AI Tools** | crossaitools.com | 21,700+ Claude Code skills, plugins, and MCP servers in one browsable directory; 300,000+ monthly visitors; daily GitHub sync; supports Claude Code, Codex, OpenCode, Cursor, and 40+ agents; search by category (frontend, backend, testing, DevOps, security). Self-reported stats track closely with Claude Marketplaces' — likely the same underlying aggregator/white-label platform rather than an independent index; check both but don't expect materially different results |
