...
| **Agensi** | agensi.io | Security-scanned marketplace — every skill passes an 8-point checklist (prompt injection, data exfiltration, secret detection, dangerous commands, obfuscation, external fetches) before listing; creator payments (70% revenue share); paid and free skills |
...
| **Tons of Skills** | tonsofskills.com | 471 plugins, ~3,069 skills, 347 agents for Claude Code; `ccpi` CLI package manager; catalog updated daily from GitHub; most comprehensive single-site collection (July 2026) |
| **claude-skills (362 skills)** | github.com/alirezarezvani/claude-skills | 362 production-ready skills across 13 AI coding agents (Claude Code, Codex CLI, Cursor, Gemini CLI, and more), spanning engineering, marketing, product, compliance, and C-level advisory domains; also includes 30+ agents, 70+ custom commands, and 644 Python CLI tools |
...
| **wshobson/agents** | github.com/wshobson/agents | Multi-harness marketplace: 94 plugins, 203 agents, 175 skills, 109 commands; native support for Claude Code, Codex CLI, Cursor, OpenCode, Gemini CLI, and GitHub Copilot — each harness gets idiomatic artifacts from one Markdown source |
...
| **Awesome Claude Plugins** | github.com/Chat2AnyLLM/awesome-claude-plugins | Curated list of Claude marketplaces and plugins — 192 marketplaces, 2,529+ plugins indexed (grows continuously); updated daily |
| **ClaudeCodexPlugins** | claudecodexplugins.com | Cross-platform directory covering both Claude Code and OpenAI Codex plugins; ranks listings by real install/click telemetry + GitHub stars (no paid placement boost); one-click install commands; $25/yr flat listing fee |
| **AgentSkill.sh** | agentskill.sh | Cross-agent skill directory (15+ platforms incl. Claude Code, Cursor, Copilot, Codex, Windsurf, Gemini CLI) with a `/learn` install skill and `ags` CLI; every skill gets a server-side 0–100 security score across 12 threat categories; separate project from Skills.sh (Vercel) — GitHub: agentskill-sh/ags |
