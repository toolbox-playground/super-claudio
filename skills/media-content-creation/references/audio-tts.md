...
ElevenLabs previewed its next-generation v4 voice model at ElevenSummit in Warsaw (June 2026). As of late August 2026, v4 still has **not been released** — no model ID, API endpoint, or release date announced; expected window is H2 2026. Preview samples demonstrated expressive delivery with emotion, intent, and accent — positioning it as "performance acting" rather than text-to-speech. Check elevenlabs.io/changelog for the GA announcement.
...
## Cartesia Sonic 3.6 — #1 Artificial Analysis Speech Arena (Elo 1,282, Aug 2026); Sonic 3.5 is the current GA pick

Cartesia shipped Sonic 3.6 in beta on August 17, 2026 (via the `sonic-preview` endpoint), just three months after Sonic 3.5. It now **leads both Artificial Analysis Speech Arena leaderboards** — Provider Voice (Elo 1,282) and the newer Controlled Voice board (Elo 1,123, a fixed 8-voice test set used across all models) — surpassing Speechify SIMBA 3.2 and Alibaba's Qwen-Audio-3.0-TTS-Plus, which had held the top two spots since mid-July 2026. The update focuses on more natural pacing, intonation, and pronunciation for conversational speech. GA is expected later in August 2026; most partner integrations and docs still point to Sonic 3.5, which itself still holds #2 on the Controlled Voice board.

Cartesia Sonic 3.5 (upgraded May 2026) remains purpose-built for real-time conversational AI with ~40ms time-to-first-audio — the standard production pick for voice agents and chatbots until 3.6 reaches GA. Sonic 3.5 improved on Sonic 3 with noticeably more natural voices, better stability through long calls, and more consistent pronunciation of difficult words and names. The Sonic 3.6 upgrade is expected to apply automatically to all existing Cartesia integrations once it exits beta.

- URL: cartesia.ai
- Free tier available
- Latency: ~40ms TTFA (Sonic 3.5, best-in-class for streaming; Sonic 3.6 not yet independently latency-benchmarked as of Aug 2026)
- Languages: 42
- Elo: **1,282 — #1 globally (Sonic 3.6, Artificial Analysis Provider Voice board, Aug 17 2026, beta)**; Sonic 3.5 holds ~1,211 (Provider Voice) and #2 on the Controlled Voice board
- Best for: voice agents, live chatbots, real-time assistants — Sonic 3.6 is now #1 on quality benchmarks as well as latency once GA; use Sonic 3.5 for production today
...
## Qwen-Audio-3.0-TTS-Plus (Alibaba) — #2 Artificial Analysis TTS Arena (Elo 1,236, since Aug 2026)

Alibaba's Qwen-Audio-3.0-TTS-Plus (distinct from the open-source Qwen3-TTS listed below) held **#1 on the Artificial Analysis Speech Arena Leaderboard** from mid-July 2026, narrowly ahead of Speechify SIMBA 3.2, until Cartesia Sonic 3.6 (Elo 1,282) surpassed both in mid-August 2026 — Qwen-Audio-3.0-TTS-Plus now sits **#2**. Delivers increased naturalness and contextually appropriate intonation; available as a proprietary cloud-hosted model via Alibaba Cloud Model Studio (DashScope). Continues Alibaba's momentum across model categories following HappyHorse and Fun-Realtime-TTS.

- **URL:** alibabacloud.com (Model Studio / DashScope)
- **Elo:** 1,236 — **#2 globally (Artificial Analysis Speech Arena, Aug 2026)**; held #1 for a month before Cartesia Sonic 3.6 (Elo 1,282) took over; narrowly above SIMBA 3.2 (1,234) with overlapping confidence intervals
...
## Speechify SIMBA 3.2 — #3 Artificial Analysis TTS Arena at $10/1M Characters (since Aug 2026)

Speechify SIMBA 3.2 (July 2026) briefly reached **#1 on the Artificial Analysis TTS Arena** before being narrowly surpassed by Qwen-Audio-3.0-TTS-Plus in mid-July 2026 (**#2, Elo 1,234**), and both were then surpassed by Cartesia Sonic 3.6 (Elo 1,282) in mid-August 2026 — SIMBA 3.2 now sits **#3 globally**. It remains, at $10/1M characters ($6/1M at the Scale tier), the lowest price among current global top-5 models by a wide margin. SIMBA 3.0 (May 2026) entered at #7 (Elo 1,159); 3.2 improved emotional control, locale coverage, and streaming latency. VentureBeat covered SIMBA 3.2 topping both Artificial Analysis and Voice Arena leaderboards on August 7, 2026, shortly before Sonic 3.6 shipped.
...
| Best blind-test naturalness (cloud) | Cartesia Sonic 3.6 (Elo 1,282, **#1** Aug 2026, beta), Qwen-Audio-3.0-TTS-Plus (Elo 1,236, **#2**), Speechify SIMBA 3.2 (Elo 1,234, **#3**), Gemini 3.1 Flash TTS (Elo ~1,214, **~#4**) |
...
| Real-time voice agent / chatbot | Cartesia Sonic 3.5 (~40ms TTFA, GA production pick) or Sonic 3.6 (beta, new #1 on quality benchmarks), Smallest.ai Lightning V3.1 (<100ms), or Inworld Realtime TTS-2 (<200ms, closed-loop, 100+ langs) |
...
| Top quality at minimum cost | Speechify SIMBA 3.2 (**#3 globally** since Aug 2026, Elo 1,234, $10/1M chars; $6/1M at Scale tier) — best price/quality by a wide margin; Cartesia Sonic 3.6 is new #1 (Elo 1,282, Aug 2026, still beta) and Qwen-Audio-3.0-TTS-Plus is #2 (Alibaba DashScope) but has slower throughput (16 chars/sec) |
...
| Highest-ranked cloud TTS by Elo, real-time streaming | Cartesia Sonic 3.6 (Elo 1,282, **#1** Aug 2026, beta, ~40ms-class latency); Qwen-Audio-3.0-TTS-Plus (Alibaba DashScope, Elo 1,236, **#2**); Speechify SIMBA 3.2 ($10/1M chars, Elo 1,234, **#3**); Gemini 3.1 Flash TTS (free in AI Studio, Elo ~1,214, ~#4); Cartesia Sonic 3.5 (free tier, GA, Elo ~1,211, ~40ms latency, stable production pick) |
