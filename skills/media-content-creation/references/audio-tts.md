...
Cartesia Sonic 3.6 (beta Aug 17, 2026; GA later in August) is now the **new #1 on the Artificial Analysis Speech Arena at Elo ~1,288** — a clear, non-overlapping lead over #2/#3 (Speechify SIMBA 3.2 and Qwen-Audio-3.0-TTS-Plus, both ~1,234–1,243). It remains purpose-built for real-time conversational AI with ~40ms time-to-first-audio, and adds Odia and Urdu (44 languages total, up from 42) plus more natural voices and better long-call stability over Sonic 3.5. The upgrade applies automatically to all existing Cartesia integrations.

- URL: cartesia.ai
- Free tier available
- Latency: ~40ms TTFA (best-in-class for streaming)
- Languages: 44
- Elo: ~1,288 — **#1 globally (Artificial Analysis Speech Arena, Aug 2026)**
...
## VUI Labs Luna-TTS — #1 HuggingFace TTS Arena, Diffusion-Based (NEW Aug 2026)

Luna-TTS from Chinese startup VUI Labs is a diffusion-based, non-autoregressive TTS model that debuted at **#1 on the HuggingFace TTS Spaces Arena** and **top-3 on the Artificial Analysis Speech Arena** (ahead of Gemini 3.1 Flash TTS), with 41.6ms first-packet latency — notable because diffusion architectures are typically slower than autoregressive ones at this latency tier.

- URL: check huggingface.co (`VUI-Labs`) for hosted demo/weights
- Latency: 41.6ms first-packet
- Architecture: Diffusion-based, non-autoregressive (distinct from most top-ranked models, which are autoregressive)
- Best for: teams wanting a non-autoregressive alternative with top-tier blind-test naturalness
- Not ideal for: production deployments needing an established commercial SLA (very new, Aug 2026 launch — verify current API/pricing before committing)
...
## Qwen-Audio-3.0-TTS-Plus (Alibaba) — Artificial Analysis TTS Arena (Elo 1,236, Mid-July 2026)

Alibaba's Qwen-Audio-3.0-TTS-Plus (distinct from the open-source Qwen3-TTS listed below) briefly held #1 on the Artificial Analysis Speech Arena Leaderboard in mid-July 2026, narrowly ahead of Speechify SIMBA 3.2 — overlapping confidence intervals made the margin close. **Cartesia Sonic 3.6 (Elo ~1,288, Aug 2026) has since taken a clear #1 lead over both.** Qwen-Audio-3.0-TTS-Plus still delivers strong naturalness and contextually appropriate intonation; available as a proprietary cloud-hosted model via Alibaba Cloud Model Studio (DashScope). Continues Alibaba's momentum across model categories following HappyHorse and Fun-Realtime-TTS.

- **URL:** alibabacloud.com (Model Studio / DashScope)
- **Elo:** 1,236 — narrowly above SIMBA 3.2 (1,234) with overlapping confidence intervals; both now behind Cartesia Sonic 3.6 (~1,288, Aug 2026)
...
| Best blind-test naturalness (cloud) | Cartesia Sonic 3.6 (Elo ~1,288, **#1** Aug 2026), Speechify SIMBA 3.2 / Qwen-Audio-3.0-TTS-Plus (Elo ~1,234–1,236, **#2/#3**, overlapping CIs), VUI Labs Luna-TTS (top-3, diffusion-based, NEW Aug 2026) |
...
| Top quality at minimum cost | Speechify SIMBA 3.2 (Elo 1,234, $10/1M chars; $6/1M at Scale tier) — best price/quality; Cartesia Sonic 3.6 is now global #1 (Elo ~1,288) but Qwen-Audio-3.0-TTS-Plus has slower throughput (16 chars/sec) if benchmarking alternatives |
...
| Highest-ranked cloud TTS by Elo, real-time streaming | Cartesia Sonic 3.6 (free tier, Elo ~1,288, **#1** Aug 2026, ~40ms latency, 44 languages); Speechify SIMBA 3.2 ($10/1M chars, Elo 1,234) and Qwen-Audio-3.0-TTS-Plus (Alibaba DashScope, Elo 1,236) trail closely behind |
