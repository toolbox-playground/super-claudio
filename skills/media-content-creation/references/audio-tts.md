...
Cartesia Sonic 3.6 (beta Aug 17, GA ~Aug 27, 2026) supersedes Sonic 3.5 just three months later. Purpose-built for real-time conversational AI with ~40ms time-to-first-audio — the standard pick for voice agents and chatbots. **Now #1 on the Artificial Analysis Speech Arena (Elo ~1,275, September 2026)**, overtaking Speechify SIMBA 3.2 and Qwen-Audio-3.0-TTS-Plus. Languages expanded to 44 (from 42 in Sonic 3.5). The upgrade applies automatically to all existing Cartesia integrations.

- URL: cartesia.ai
- Free tier available
- Latency: ~40ms TTFA (best-in-class for streaming)
- Languages: 44
- Elo: ~1,275 — **#1 globally (Artificial Analysis Speech Arena, September 2026)**
- Best for: voice agents, live chatbots, real-time assistants — now leads on both quality and latency
...
## Amazon Nova 2 Sonic — Native Speech-to-Speech (NEW Sept 2026)

Amazon's Nova 2 Sonic (September 2026) is a native speech-to-speech model — like GPT-Realtime-2, it processes and generates audio directly rather than chaining STT→LLM→TTS, reducing latency and preserving tone/prosody through the pipeline. New entrant; positioned for AWS-ecosystem voice agents.

- **URL:** aws.amazon.com/bedrock (Nova model family)
- **Best for:** Developers already building on AWS Bedrock who want a native speech-to-speech model without stitching separate STT/TTS services
- **Not ideal for:** Non-AWS deployments where Cartesia, ElevenLabs, or OpenAI's Realtime API are simpler to integrate
...
Deepgram Aura-2 targets enterprises building production voice systems where uptime, transparent pricing, and consistent low latency matter more than theatrical expressiveness. Sub-200ms TTFB (90ms optimized), 40+ English voices, 10+ Spanish voices with regional accents. **Deepgram Flux TTS** (GA Aug 12, 2026) is a newer, conversation-native engine built specifically for voice agents — designed around turn-taking and interruption handling rather than single-shot narration; complements Aura-2 rather than fully replacing it.

- URL: deepgram.com
- Free start: $200 in free credits (no credit card required)
- Pricing: $0.030 per 1,000 characters (~$1.80/hour of audio) for Aura-2
- Languages: 7 (English, Spanish + others)
- Best for: enterprise voice agents, high-volume production systems, call center bots; Flux specifically for conversational turn-taking
...
## Qwen-Audio-3.0-TTS-Plus (Alibaba) — Formerly #1, Now #3 Artificial Analysis TTS Arena (Sept 2026)

Alibaba's Qwen-Audio-3.0-TTS-Plus (distinct from the open-source Qwen3-TTS listed below) briefly held #1 on the Artificial Analysis Speech Arena Leaderboard in mid-July 2026; as of September 2026 it sits **#3**, behind Cartesia Sonic 3.6 (#1) and Inworld Realtime TTS-2 (#2). Delivers increased naturalness and contextually appropriate intonation; available as a proprietary cloud-hosted model via Alibaba Cloud Model Studio (DashScope). Continues Alibaba's momentum across model categories following HappyHorse and Fun-Realtime-TTS.

- **URL:** alibabacloud.com (Model Studio / DashScope)
- **Elo:** 1,236 — **#3 globally (Artificial Analysis Speech Arena, September 2026)**; held #1 briefly in mid-July 2026 before Speechify SIMBA 3.2, Cartesia Sonic 3.6, and Inworld Realtime TTS-2 rose above it
...
Speechify SIMBA 3.2 (July 2026) briefly reached #1–#2 on the Artificial Analysis TTS Arena during July 2026; by September 2026 it sits **#4** as Cartesia Sonic 3.6, Inworld Realtime TTS-2, and Qwen-Audio-3.0-TTS-Plus rose above it — at $10/1M characters ($6/1M at the Scale tier), still the lowest price among the current global top-5 models. SIMBA 3.0 (May 2026) entered at #7 (Elo 1,159); 3.2 improved emotional control, locale coverage, and streaming latency.
...
| Best blind-test naturalness (cloud) | Cartesia Sonic 3.6 (Elo ~1,275, **#1** Sept 2026), Inworld Realtime TTS-2 (Elo ~1,244, **#2**), Qwen-Audio-3.0-TTS-Plus (Elo 1,236, **#3**), Speechify SIMBA 3.2 (Elo 1,234, **#4**) |
...
| Real-time voice agent / chatbot | Cartesia Sonic 3.6 (~40ms TTFA, **#1 globally**), Smallest.ai Lightning V3.1 (<100ms), Inworld Realtime TTS-2 (<200ms, closed-loop, 100+ langs), or Deepgram Flux TTS (turn-taking native) |
...
| Top quality at minimum cost | Speechify SIMBA 3.2 (**#4 globally** Sept 2026, Elo 1,234, $10/1M chars; $6/1M at Scale tier) — best price/quality; Cartesia Sonic 3.6 is now #1 but has no published low-cost tier to match SIMBA's $6-10/1M chars |
...
| Highest-ranked cloud TTS by Elo, real-time streaming | Cartesia Sonic 3.6 (free tier, Elo ~1,275, **#1** Sept 2026, ~40ms latency, 44 langs); Inworld Realtime TTS-2 (Elo ~1,244, **#2**); Qwen-Audio-3.0-TTS-Plus (Alibaba DashScope, Elo 1,236, **#3**); Speechify SIMBA 3.2 ($10/1M chars, Elo 1,234, **#4**) |
