...
## Qwen-Audio-3.0-TTS-Plus (Alibaba) — Top-2 Artificial Analysis TTS Arena (Elo ~1,227–1,236)

Alibaba's Qwen-Audio-3.0-TTS-Plus (distinct from the open-source Qwen3-TTS listed below) reached **#1 on the Artificial Analysis Speech Arena Leaderboard** in mid-July 2026, narrowly ahead of Speechify SIMBA 3.2 — overlapping confidence intervals make the margin close. **As of early August 2026, the #1/#2 spot has flipped back to SIMBA 3.2** per Speechify's own announcement and third-party coverage (martechseries.com); treat Qwen and SIMBA as a statistical tie (~Elo 1,227–1,236) that reshuffles week to week rather than a settled ranking. Delivers increased naturalness and contextually appropriate intonation; available as a proprietary cloud-hosted model via Alibaba Cloud Model Studio (DashScope). Continues Alibaba's momentum across model categories following HappyHorse and Fun-Realtime-TTS.
...
## Speechify SIMBA 3.2 — Top-2 Artificial Analysis TTS Arena at $10/1M Characters (Elo ~1,229–1,234)

Speechify SIMBA 3.2 (July 2026) reached **#1 on the Artificial Analysis TTS Arena**, was narrowly surpassed by Qwen-Audio-3.0-TTS-Plus in mid-July 2026 (**#2, Elo 1,234**), then **reclaimed #1 in early August 2026** per Speechify's own announcement — overlapping confidence intervals make the two effectively co-equal on quality. At $10/1M characters ($6/1M at the Scale tier), it remains the lowest price among current global top-5 models. SIMBA 3.0 (May 2026) entered at #7 (Elo 1,159); 3.2 improved emotional control, locale coverage, and streaming latency.
...
## Luna TTS (Pixa AI) — New Top-5 Entrant on Artificial Analysis Speech Arena

Luna, a speech-to-speech conversational voice model from Pixa AI (Jaipur, India), has risen into the **Artificial Analysis Speech Arena top 5 (Elo ~1,205, early August 2026)**, pushing Cartesia Sonic 3.5 down to roughly #5. It is speech-to-speech rather than pure text-to-speech, so it's a conversational-agent pick, not a narration/TTS replacement.

- **URL:** pixa.ai (not independently verified this scan — WebFetch was unavailable; confirm before quoting pricing)
- **Best for:** real-time conversational voice agents
- **Not ideal for:** offline/batch narration or non-conversational voiceover (use a dedicated TTS model like Cartesia, Inworld, or Speechify for those)
...
| Top quality at minimum cost | Speechify SIMBA 3.2 ($10/1M chars; $6/1M at Scale tier) — best price/quality, and swaps #1/#2 week to week with Qwen-Audio-3.0-TTS-Plus (~Elo 1,227–1,236, statistical tie — see note above); Qwen has slower throughput (16 chars/sec) |
...
| Highest-ranked cloud TTS by Elo, real-time streaming | Qwen-Audio-3.0-TTS-Plus (Alibaba DashScope) and Speechify SIMBA 3.2 ($10/1M chars) — statistically tied at #1/#2, ~Elo 1,227–1,236, trading places week to week; Gemini 3.1 Flash TTS (free in AI Studio, Elo ~1,214, ~#3); Cartesia Sonic 3.5 (free tier, ~#4); Luna TTS (Pixa AI) newly in the top 5 as of early Aug 2026 |
