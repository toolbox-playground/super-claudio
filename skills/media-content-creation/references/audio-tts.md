...
ElevenLabs previewed its next-generation v4 voice model at ElevenSummit in Warsaw (June 2026). As of September 2026, v4 still has **not been released** — no model ID, API endpoint, or confirmed release date; it was informally expected in H2 2026 but ElevenLabs has not confirmed a date. Preview samples demonstrated expressive delivery with emotion, intent, and accent — positioning it as "performance acting" rather than text-to-speech. Check elevenlabs.io/changelog for the GA announcement.
...
- **Elo:** ~1,208 on Artificial Analysis Speech Arena — **#3 globally (July 2026)**; held #1 briefly in June 2026 (Elo 1,219) before Gemini 3.1 Flash TTS and Cartesia Sonic 3.5 surpassed it
...
## Cartesia Sonic 3.6 — #1 Artificial Analysis TTS Arena (Provider Voice), Real-Time Conversational TTS (UPDATED Aug 2026)

Cartesia Sonic 3.6 (released August 27, 2026, just three months after Sonic 3.5) retook the #1 spot on the Artificial Analysis Provider Voice Speech Arena, surpassing Speechify SIMBA 3.2 and Alibaba's Qwen-Audio-3.0-TTS-Plus. In blind head-to-head tests, listeners preferred Sonic 3.6 over Sonic 3.5 up to 93% of the time across 15 locales, and preferred it over ElevenLabs Eleven v3 92% of the time in US English. It runs on a state-space-model architecture rather than transformers. The upgrade applies automatically to all existing Cartesia integrations.

- URL: cartesia.ai
- Free tier available
- Latency: sub-90ms model-only latency per Cartesia (vendor figure); independent benchmarks measure ~165–190ms real-world TTFA once network overhead is included
- Languages: 44 (61 locales, including new Odia, Urdu, and Hinglish support)
- Elo: **~1,277–1,282 — #1 globally on Provider Voice (Artificial Analysis Speech Arena, Sept 2026)**; on the separate Controlled Voice arena (cloned common voice), Inworld Realtime TTS-2 edged ahead of Sonic 3.6 by a few Elo points in early September 2026 — the two models currently split the two leaderboards
...
## Google Gemini 3.8 Live / 3.8 Live Extended Thinking — Voice-to-Voice Conversation Model (NEW Sept 2026)

Google shipped Gemini 3.8 Live and Gemini 3.8 Live Extended Thinking on September 15, 2026 — not a dedicated TTS model but a voice-to-voice conversation model (same category as OpenAI's GPT-Realtime-2 / GPT-Live-1, above) that sustains real-time spoken dialogue across 97 languages, understands camera/video input, and runs tool calls while it keeps talking. Extended Thinking adds multi-step reasoning for complex requests, billed with a thinking-token surcharge. It tops Artificial Analysis's Speech-to-Speech Quality Index as of launch. This supersedes Gemini 3.1/3.5 Flash TTS (below) as Google's flagship conversational voice offering; Flash TTS remains Google's dedicated text-to-speech (non-conversational) model.

- **URL:** blog.google (Gemini models) | ai.google.dev
- **Pricing (3.8 Live):** $0.005/min audio input, $0.018/min audio output (~$1.38/hour of conversation)
- **Pricing (3.8 Live Extended Thinking):** $3.00/1M audio input tokens, $4.50/1M text output tokens, $12.00/1M audio output tokens (thinking tokens billed as output)
- **Languages:** 97, with mid-sentence language switching
- **Best for:** Real-time multimodal voice agents needing vision + tool calls + reasoning in one model; Google/Gemini ecosystem developers
- **Not ideal for:** Pure TTS pipelines or budget-sensitive batch narration — use Gemini Flash TTS, Kokoro, or edge-tts for those
...
## Deepgram Flux TTS — Conversation-Aware TTS for Voice Agents (NEW Aug 2026)

Deepgram launched Flux TTS on August 12, 2026, as a "conversation-native" companion to Aura-2: instead of synthesizing each line in isolation, it reads the whole conversation so far — carrying tone and context from the first turn to the last — and reliably pronounces alphanumerics, drug names, account numbers, dates, and currency without SSML or style-tag prompting. Deploys in the cloud, private cloud, or fully on-prem for regulated workloads (healthcare, finance, government).

- **URL:** deepgram.com/product/text-to-speech/flux
- **Free tier:** Was free through September 12, 2026 (launch promo); standard pay-as-you-go pricing began September 13, 2026
- **Pricing:** $0.045 per 1,000 characters; Deepgram matches spend with bonus credits (up to $500) on Pay-As-You-Go and Growth plans
- **Best for:** Enterprise voice agents needing accurate entity pronunciation and full-conversation context without prompt engineering; regulated industries needing on-prem deployment
- **Not ideal for:** Cheapest high-volume batch generation (use Speechmatics or Kokoro); expressive/theatrical delivery (use Hume Octave 2)
...
## Qwen-Audio-3.0-TTS-Plus (Alibaba) — #2 Artificial Analysis TTS Arena (Elo ~1,259, Sept 2026)

Alibaba's Qwen-Audio-3.0-TTS-Plus (distinct from the open-source Qwen3-TTS listed below) briefly held #1 on the Artificial Analysis Speech Arena Leaderboard in mid-July 2026 but was overtaken by Cartesia Sonic 3.6 in late August 2026; as of September 2026 it sits **#2 on Provider Voice** (Elo ~1,259), ahead of Inworld Realtime TTS-2 and Speechify SIMBA 3.2. Delivers increased naturalness and contextually appropriate intonation; available as a proprietary cloud-hosted model via Alibaba Cloud Model Studio (DashScope). Continues Alibaba's momentum across model categories following HappyHorse and Fun-Realtime-TTS.

- **URL:** alibabacloud.com (Model Studio / DashScope)
- **Elo:** ~1,259 — **#2 globally on Provider Voice (Artificial Analysis Speech Arena, Sept 2026)**, behind Cartesia Sonic 3.6 (~1,277–1,282)
- **Free tier:** Alibaba Cloud trial credits on new accounts; no permanent free tier
- **Generation speed:** ~16 chars/sec — significantly slower than Sonic 3.6 and SIMBA 3.2 (~30 chars/sec); note for throughput-sensitive pipelines
- **Best for:** Applications prioritizing top benchmark quality; developers already in the Alibaba Cloud / DashScope ecosystem
- **Not ideal for:** Ultra-low-latency agents (use Cartesia Sonic 3.6); high-throughput batch generation (use SIMBA 3.2 or Kokoro); offline/on-device deployment (use NeuTTS Air)

## Speechify SIMBA 3.2 — Cheapest Global Top-5 Model at $10/1M Characters (Updated Sept 2026)

Speechify SIMBA 3.2 (July 2026) briefly reached #1 on the Artificial Analysis TTS Arena before Qwen-Audio-3.0-TTS-Plus and then Cartesia Sonic 3.6 overtook it; as of September 2026 it sits **~#4 on Provider Voice** (Elo ~1,239–1,241) — still at $10/1M characters ($6/1M at the Scale tier), the lowest price among the current global top-5 models. On September 10, 2026, Speechify removed the earlier voice allow-list: SIMBA 3.2 now serves every English voice in its catalogue (including cloned voices), not just the built-in `*_32` example voices; non-English voices still require SIMBA 3.0.
...
- **Features:** Fine-grained emotional control at the prosody level (rhythmic and tonal patterns), SSML prosody support, zero-shot voice cloning from short reference clips, all English voices now unrestricted (Sept 2026), 30+ locales, mixed-language input handled automatically
- **Best for:** Cost-sensitive high-volume production TTS needing near-top quality; enterprises benchmarking ElevenLabs alternatives at 10× lower price
...
StepAudio 2.5 TTS from StepFun (Shanghai) ranked #3 on the Artificial Analysis Speech Arena at launch (Elo ~1,187, May 2026); by July 2026 it sits ~#7 as Gemini 3.1 Flash TTS, Cartesia Sonic 3.5, Fun-Realtime-TTS, Inworld TTS-2, Inworld TTS-1.5 Max, and xAI Grok TTS all rank above it. The key differentiator: it is the first TTS model to integrate full contextual understanding into the generation pipeline, so it doesn't just read text, it *performs* it.
...
- **Elo:** ~1,187 on Artificial Analysis Speech Arena — #3 globally at launch (May 2026), ~#7 globally (July 2026) as newer models rose above it; still above ElevenLabs Eleven v3 (~1,178)
...
- **Not ideal for:** Ultra-low-latency real-time agents where Cartesia Sonic 3.6's sub-90ms latency is needed; theatrical emotional delivery (use Hume Octave 2)

## dots.tts (rednote-hilab) — Open-Source Zero-Shot Voice Cloning, Apache 2.0 (NEW June 2026)

dots.tts, from Hilab (the research division of Chinese social platform RedNote/Xiaohongshu, with Shanghai Jiao Tong University), released June 9, 2026, is a 2B-parameter fully continuous autoregressive TTS model that clones a voice from a short reference clip with no fine-tuning. It ranked #1 in an independent blind comparison of 11 voice-cloning systems (August 2026), ahead of MOSS-TTS v1.5.

```bash
git clone https://github.com/rednote-hilab/dots.tts
```

- **URL / GitHub:** github.com/rednote-hilab/dots.tts | **HuggingFace:** `rednote-hilab/dots.tts-soar` (clone), `dots.tts-base` (fine-tune), `dots.tts-mf` (real-time)
- **License:** Apache 2.0 (commercial use permitted)
- **Parameters:** 2B
- **Audio quality:** Native 48kHz output
- **Languages:** Benchmarked across 24 languages (83.9 average multilingual speaker similarity); broader coverage claimed via its language-model backbone
- **Latency:** First-chunk latency of ~68–69ms (text-only mode) or ~204–225ms (voice-cloning mode) on H800 GPU, per the project README (real-time `-mf`/MeanFlow variant fastest)
- **Voice cloning:** Zero-shot from a short reference clip, no fine-tuning required
- **Best for:** Self-hosted, commercial-use-permitted voice cloning needing top blind-test quality; developers wanting a real-time streaming variant out of the box
- **Not ideal for:** Turnkey cloud API (no hosted offering yet — self-host only); English-only lightweight deployment (use Kokoro or NeuTTS Air)
...
- **Not ideal for:** Ultra-low-latency real-time agents where Cartesia Sonic 3.6's sub-90ms latency is needed; English-only lightweight deployment (use Kokoro or NeuTTS Air for those)
...
- **Not ideal for:** Budget-sensitive high-volume use (use Speechmatics $0.011/1K chars or Kokoro at no cost); theatrical emotional range (use Hume Octave 2); top-5 quality ranking (Cartesia Sonic 3.6, Qwen-Audio-3.0-TTS-Plus, and Inworld Realtime TTS-2 now rank above it)
...
| Best blind-test naturalness (cloud) | Cartesia Sonic 3.6 (Elo ~1,280, **#1 Provider Voice** Sept 2026), Qwen-Audio-3.0-TTS-Plus (Elo ~1,259, **#2**), Inworld Realtime TTS-2 (Elo ~1,247–1,252, **#3**, and **#1 on the separate Controlled Voice arena**), Speechify SIMBA 3.2 (Elo ~1,239–1,241, **#4**) |
...
| Real-time voice agent / chatbot | Cartesia Sonic 3.6 (#1 Provider Voice Elo, 44 languages), Smallest.ai Lightning V3.1 (<100ms), or Inworld Realtime TTS-2 (#1 Controlled Voice Elo, closed-loop, 100+ langs) |
| Voice agent needing GPT-5-class reasoning in audio (complex tool calls, multi-turn logic) | OpenAI GPT-Realtime-2 ($32/1M in + $64/1M out; 128K context, parallel tool calls, interruption handling — expensive; not a substitute for pure TTS) or Google Gemini 3.8 Live Extended Thinking ($3/1M audio in, $12/1M audio out; vision + tool calls, 97 languages) |
| Conversation-aware TTS with accurate entity/number pronunciation, on-prem option | Deepgram Flux TTS ($0.045/1K chars, cloud/private-cloud/on-prem) |
| Open-source voice cloning, top blind-test quality, self-hosted, Apache 2.0 | dots.tts (rednote-hilab, 2B params, 48kHz, github.com/rednote-hilab/dots.tts) |
...
| Top quality at minimum cost | Speechify SIMBA 3.2 (**~#4 globally** Sept 2026, Elo ~1,240, $10/1M chars; $6/1M at Scale tier, now all English voices unrestricted) — best price/quality; Cartesia Sonic 3.6 and Qwen-Audio-3.0-TTS-Plus rank higher but at less aggressive pricing |
| Conversational AI with tone/context awareness across turns | Inworld Realtime TTS-2 (closed-loop, adapts to prior audio, 100+ langs, #1 Controlled Voice arena Sept 2026) |
...
| Highest-ranked cloud TTS by Elo, real-time streaming | Cartesia Sonic 3.6 (**#1 Provider Voice**, Elo ~1,280, Sept 2026, ~90ms model latency, 44 langs); Qwen-Audio-3.0-TTS-Plus (Elo ~1,259, #2); Inworld Realtime TTS-2 (#1 Controlled Voice arena) |
| Voice-to-voice conversational model with vision + reasoning | Google Gemini 3.8 Live / 3.8 Live Extended Thinking (97 languages, ~$1.38/hr, Sept 2026) |
