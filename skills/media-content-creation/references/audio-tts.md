...
**Note (2026):** Microsoft's underlying Read Aloud API now enforces a 10-minute audio limit per request plus other rate limits; chunk long text into segments under ~2,500 words / 15,000 characters per call to avoid timeouts. Python/CLI `edge-tts` usage is unaffected by the separate browser-only `Sec-WebSocket-Version` restriction.
...
Cartesia Sonic 3.6 (beta Aug 17, GA ~Aug 27, 2026) supersedes Sonic 3.5 as Cartesia's flagship — purpose-built for real-time conversational AI with best-in-class latency, now also leading on raw quality. Preferred over Eleven v3 in ~92% of US-English blind tests (up to 93% across 15 locales). The upgrade applies automatically to all existing Cartesia integrations.

- URL: cartesia.ai
- Free tier available
- Latency: ~40ms TTFA (best-in-class for streaming)
- Languages: 44 (up from 42 in Sonic 3.5)
- Elo: 1,277 — **#1 globally on both Provider Voice and Controlled Voice arenas (Artificial Analysis Speech Arena, Sept 2026)** — rare to lead both simultaneously
- Pricing: ~$49/1M characters via Cartesia platform
- Best for: real-time conversational agents; now competitive with the best long-form models too, not just the fastest option
- Not ideal for: budget-sensitive pipelines where Kokoro/edge-tts/Speechify SIMBA are cheaper
...
## Deepgram Flux TTS — Conversation-Native TTS for Voice Agents (Aug 2026)

Deepgram Flux TTS (launched Aug 12, 2026) sits alongside Deepgram's existing Aura-2 (below) as the conversation-aware option: it tracks conversational context across turns rather than treating each TTS request as an isolated static string.

- **URL:** deepgram.com/product/text-to-speech/flux
- **Latency:** as low as 80ms; 24kHz output
- **Pricing:** free trial ended Sept 12, 2026; standard pricing now $0.045/1,000 characters (free-trial concurrency was up to 45 streaming connections, 5 in EU/AU)
- **Best for:** enterprise voice agents needing conversation-aware delivery in the Deepgram ecosystem
- **Not ideal for:** budget pipelines (pricier per-character than Aura-2's $0.030/1,000) or one-shot narration — use Aura-2 for those

## Breeze TTS 2 (BreezeBlue) — #1 Open-Weight TTS on Artificial Analysis (Aug 2026)

Breeze TTS 2 (released Aug 25, 2026) is the new #1 among open-weight models on the Artificial Analysis Speech Arena, surpassing Fish Audio S2 Pro by ~90 Elo points.

- **URL / GitHub:** github.com/breezeblue-ai/breeze-tts | **HuggingFace:** `BreezeBlue/Breeze-TTS-2`
- **License:** inference code Apache 2.0, but **model weights are research/non-commercial only** — commercial use requires separate written authorization from RESONIA, INC. (do not treat as freely commercial like Kokoro/NeuTTS Air/Qwen3-TTS/ZONOS2)
- **Elo:** 1,215 on Artificial Analysis Speech Arena — #1 among open-weight models, ~#6 overall vs. closed models
- **Languages:** vendor claims 50; other reporting describes English + Chinese as the primary supported pair — confirm firsthand before publishing broad language claims
- **Latency:** sub-40ms TTFA claimed (H100 fast-path); hosted API TTFA runs closer to ~134ms p50
- **Features:** voice cloning, natural-language voice design, "directed delivery" control, streaming
- **Best for:** best-in-class open-weight quality where research/non-commercial licensing is acceptable, or where a commercial license is separately negotiated
...
## Speechify SIMBA 3.2 — #4 Artificial Analysis TTS Arena at $10/1M Characters (Sept 2026)

Speechify SIMBA 3.2 (July 2026) reached #1 on the Artificial Analysis TTS Arena in mid-July 2026, then Qwen-Audio-3.0-TTS-Plus, Cartesia Sonic 3.6, and Inworld Realtime TTS-2 each rose above it through August (now **#4, Elo 1,241**) — at $10/1M characters ($6/1M at the Scale tier), still the lowest price among the current global top 5. SIMBA 3.0 (May 2026) entered at #7 (Elo 1,159); 3.2 improved emotional control, locale coverage, and streaming latency.
...
| Best blind-test naturalness (cloud) | Cartesia Sonic 3.6 (Elo 1,277, **#1** Sept 2026), Qwen-Audio-3.0-TTS-Plus (Elo 1,259, **#2**), Inworld Realtime TTS-2 (Elo 1,247, **#3**), Speechify SIMBA 3.2 (Elo 1,241, **#4**) |
| Best open-weight naturalness (self-hosted) | Breeze TTS 2 (Elo 1,215, **#1 open-weight**, Aug 2026 — research/non-commercial license; commercial use needs separate authorization) |
...
| Real-time voice agent / chatbot | Cartesia Sonic 3.6 (~40ms TTFA, now also #1 on quality), Smallest.ai Lightning V3.1 (<100ms), Deepgram Flux TTS (~80ms, conversation-aware context), or Inworld Realtime TTS-2 (<200ms, closed-loop, 100+ langs) |
...
| Top quality at minimum cost | Speechify SIMBA 3.2 (**#4 globally** Sept 2026, Elo 1,241, $10/1M chars; $6/1M at Scale tier) — best price/quality among the top 5; Cartesia Sonic 3.6 is now #1 but has no comparably cheap tier, and Qwen-Audio-3.0-TTS-Plus (#2, Alibaba DashScope) has slower throughput (16 chars/sec) |
...
| Highest-ranked cloud TTS by Elo, real-time streaming | Cartesia Sonic 3.6 (free tier, Elo 1,277, **#1** Sept 2026, ~40ms latency); Qwen-Audio-3.0-TTS-Plus (Alibaba DashScope, Elo 1,259, **#2**); Inworld Realtime TTS-2 (Elo 1,247, **#3**, closed-loop); Speechify SIMBA 3.2 ($10/1M chars, Elo 1,241, **#4**) |
