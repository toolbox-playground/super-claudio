...
Cartesia Sonic 3.6 (beta Aug 17, GA Aug 27, 2026 — succeeds Sonic 3.5) is purpose-built for real-time conversational AI with ~40ms time-to-first-audio — the standard pick for voice agents and chatbots. Sonic 3.6 improves on 3.5 with noticeably more natural voices, better stability through long calls, and more consistent pronunciation of difficult words and names. **Now #1 on both Artificial Analysis Speech Arena leaderboards (Provider Voice and Controlled Voice), Elo ~1,283–1,286** — up from #2 at Sonic 3.5. Qwen-Audio-3.0-TTS-Plus and Speechify SIMBA 3.2 are now statistically tied for #2/#3 (~1,237–1,238 Elo). The upgrade applies automatically to all existing Cartesia integrations.

- URL: cartesia.ai
- Free tier: check current plan page — Sonic 3.6 is reported pricier than 3.5 (~$49/1M characters), re-verify free-tier availability before relying on it
- Latency: ~40ms TTFA (best-in-class for streaming)
- Languages: 44
- Elo: ~1,283–1,286 — **#1 globally (Artificial Analysis Speech Arena, both leaderboards, Aug 27 2026)**
...
## Qwen-Audio-3.0-TTS-Plus (Alibaba) — Tied #2/#3 Artificial Analysis TTS Arena (Elo ~1,237, Aug 2026)

Alibaba's Qwen-Audio-3.0-TTS-Plus (distinct from the open-source Qwen3-TTS listed below) held #1 on the Artificial Analysis Speech Arena Leaderboard from mid-July 2026 until Cartesia Sonic 3.6 took the top spot on Aug 27, 2026 — Qwen-Audio-3.0-TTS-Plus is now statistically tied with Speechify SIMBA 3.2 for #2/#3. Delivers increased naturalness and contextually appropriate intonation; available as a proprietary cloud-hosted model via Alibaba Cloud Model Studio (DashScope). Continues Alibaba's momentum across model categories following HappyHorse and Fun-Realtime-TTS.

- **URL:** alibabacloud.com (Model Studio / DashScope)
- **Elo:** ~1,237 — **tied #2/#3 globally (Artificial Analysis Speech Arena, Aug 2026)**; held #1 mid-July–Aug 27 2026 before Cartesia Sonic 3.6 surpassed it
...
## Speechify SIMBA 3.2 — Tied #2/#3 Artificial Analysis TTS Arena at $10/1M Characters (Aug 2026)

Speechify SIMBA 3.2 (July 2026) reached #1 on the Artificial Analysis TTS Arena before being narrowly surpassed by Qwen-Audio-3.0-TTS-Plus in mid-July 2026, and both are now statistically tied for **#2/#3 (Elo ~1,238)** since Cartesia Sonic 3.6 took #1 on Aug 27, 2026 — at $10/1M characters ($6/1M at the Scale tier), still the lowest price among the current global top tier. SIMBA 3.0 (May 2026) entered at #7 (Elo 1,159); 3.2 improved emotional control, locale coverage, and streaming latency, rising to #1 by mid-July 2026.
...
## Luna TTS (VUI Labs) — #1 HuggingFace TTS Arena, #3 Artificial Analysis Speech Arena

Luna TTS from Chinese startup VUI Labs ranks #1 on the HuggingFace TTS Spaces Arena and #3 on the Artificial Analysis Speech Arena (Elo ~1,223, Aug 2026), placing it just behind the Qwen-Audio-3.0-TTS-Plus/SIMBA 3.2 tier.

- **URL:** vuilabs.ai
- **Pricing:** ~$80/1M characters (no permanent free tier confirmed)
- **Best for:** Applications prioritizing top blind-preference naturalness scores
- **Not ideal for:** Budget-sensitive pipelines (use Speechify SIMBA 3.2 or Kokoro instead)

## Breeze TTS 2 (Breeze Blue) — #1 Open-Weight Model, Real-Time Interactive Voice

Breeze TTS 2 (launched Aug 17, 2026) is the top-ranked **open-weight** model on the Artificial Analysis TTS leaderboard, purpose-built for real-time interactive use cases like games and AI companions.

- **License:** Open-weight
- **Best for:** Self-hosted real-time interactive/game/companion voice where open weights matter
- **Not ideal for:** Chasing the absolute top Elo (proprietary Cartesia Sonic 3.6, Qwen-Audio-3.0-TTS-Plus, and SIMBA 3.2 still rank higher overall)
...
| Best blind-test naturalness (cloud) | Cartesia Sonic 3.6 (Elo ~1,284, **#1** Aug 27 2026), Qwen-Audio-3.0-TTS-Plus & Speechify SIMBA 3.2 (Elo ~1,237–1,238, **tied #2/#3**), Luna TTS (Elo ~1,223, **#3**) |
...
| Top quality at minimum cost | Speechify SIMBA 3.2 (**tied #2/#3 globally** Aug 2026, Elo ~1,238, $10/1M chars; $6/1M at Scale tier) — best price/quality; Cartesia Sonic 3.6 is new #1 but pricier (~$49/1M chars); Qwen-Audio-3.0-TTS-Plus ties SIMBA but has slower throughput (16 chars/sec) |
...
| Highest-ranked cloud TTS by Elo, real-time streaming | Cartesia Sonic 3.6 (Elo ~1,284, **#1**, Aug 27 2026, ~40ms latency, re-check free tier); Qwen-Audio-3.0-TTS-Plus (Alibaba DashScope, Elo ~1,237, **tied #2/#3**); Speechify SIMBA 3.2 ($10/1M chars, Elo ~1,238, **tied #2/#3**); Luna TTS (vuilabs.ai, Elo ~1,223, #3) |
