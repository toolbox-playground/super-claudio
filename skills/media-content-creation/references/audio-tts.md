...
## ElevenLabs v4 — Previewed at ElevenSummit Warsaw (Still Not Released as of Sept 2026)

ElevenLabs previewed its next-generation v4 voice model at ElevenSummit in Warsaw (June 2026). As of mid-September 2026, v4 **still has not shipped** — checked the ElevenLabs changelog through its August 17, 2026 entry and found no model ID, API endpoint, or GA date for v4; recent changelog activity has been agent/telephony features (Exotel integration, agent transfers, workspace API key revoke), not a new voice model. Public trackers now estimate H2 2026 for a release. Preview samples demonstrated expressive delivery with emotion, intent, and accent — positioning it as "performance acting" rather than text-to-speech. Check elevenlabs.io/changelog for the GA announcement.
...
Inworld Realtime TTS-2 (launched May 5, 2026) is a fundamentally different architecture from TTS-1.5: it takes the actual audio of prior conversation turns as input — not just text — so it understands tone, pacing, and emotional context from everything that was said before. It has since climbed to **#2 on the Artificial Analysis Provider Voice arena** (Elo ~1,250, Sept 2026; up from #4 at Elo 1,203 in July), behind only Cartesia Sonic 3.6 — and it holds **#1 on the separate Controlled Voice arena** (Elo ~1,123, which clones every model onto the same reference voices to score the underlying engine rather than the shipped voice).
...
- **Elo:** ~1,208 on Artificial Analysis Speech Arena — was #3 globally in July 2026; by Sept 2026 it has dropped further outside the top 5 as Cartesia Sonic 3.6, Inworld Realtime TTS-2, Speechify SIMBA 3.2, Qwen-Audio-3.0-TTS-Plus, and new entrant Luna-TTS all rank above it — check the live leaderboard for its current position
...
Google's Gemini 3.1 Flash TTS (launched April 15, 2026) delivers natural, expressive speech with inline audio tags for style and pacing control — and is currently free in AI Studio during preview; still in preview as of August 2026, no GA date announced. It held #1 on the Artificial Analysis TTS Arena through May 2026 (Elo ~1,217) but has since fallen well outside the top 5 as Cartesia Sonic 3.6, Inworld Realtime TTS-2, Speechify SIMBA 3.2, Qwen-Audio-3.0-TTS-Plus, and Luna-TTS all now rank above it.
...
- **Elo:** ~1,217 on Artificial Analysis TTS Arena — held #1 through May 2026; by Sept 2026 sits outside the top 5 as Cartesia Sonic 3.6 (~1,280), Inworld Realtime TTS-2 (~1,250), SIMBA 3.2 (~1,237), Qwen-Audio-3.0-TTS-Plus (~1,235), and Luna-TTS all rank above it — still a strong free option, just no longer top-tier by Elo
...
## Cartesia Sonic 3.6 — NEW #1 Artificial Analysis Speech Arena (Aug 2026)

Cartesia shipped Sonic 3.6 in beta on August 18, 2026, and it now leads **both** Artificial Analysis speech arenas — #1 on the Provider Voice arena (Elo ~1,280, dethroning Qwen-Audio-3.0-TTS-Plus and Speechify SIMBA 3.2) and #1 on the Controlled Voice arena (Elo ~1,123, the board that clones every model onto the same reference voices to score the synthesis engine itself). It's built on a State Space Model architecture (not a Transformer), which is how it holds sub-90ms time-to-first-audio while also topping the quality leaderboard — previously a latency/quality trade-off. Expanded to 44 languages (up from 42 in Sonic 3.5). The upgrade applies automatically to existing Cartesia integrations.

- URL: cartesia.ai
- Free tier available
- Latency: <90ms TTFA (best-in-class for streaming)
- Languages: 44
- Architecture: State Space Model (SSM), not Transformer-based
- Elo: ~1,280 Provider Voice arena, ~1,123 Controlled Voice arena — **#1 on both, Artificial Analysis Speech Arena (Aug 2026)**; exact Elo shifts as new models are added, check artificialanalysis.ai/text-to-speech/arena for the live number
- Best for: voice agents, live chatbots, real-time assistants — now the top pick on quality benchmarks as well as latency, not just a latency specialist
...
## Qwen-Audio-3.0-TTS-Plus (Alibaba) — Former #1, Now Top-5 Artificial Analysis Speech Arena (Elo ~1,235, Sept 2026)

Alibaba's Qwen-Audio-3.0-TTS-Plus (distinct from the open-source Qwen3-TTS listed below) held **#1 on the Artificial Analysis Speech Arena** in mid-July 2026 but has since been passed by Cartesia Sonic 3.6, Inworld Realtime TTS-2, and Speechify SIMBA 3.2 — it now sits roughly **#4** (Elo ~1,235, essentially unchanged; it was overtaken by newer models, not by losing ground itself). Delivers increased naturalness and contextually appropriate intonation; available as a proprietary cloud-hosted model via Alibaba Cloud Model Studio (DashScope).

- **URL:** alibabacloud.com (Model Studio / DashScope)
- **Elo:** ~1,235 — **~#4 globally (Artificial Analysis Speech Arena, Sept 2026)**; was #1 in mid-July 2026 before Cartesia Sonic 3.6, Inworld Realtime TTS-2, and SIMBA 3.2 rose above it
...
- **Best for:** Applications prioritizing top-5 benchmark quality; developers already in the Alibaba Cloud / DashScope ecosystem
- **Not ideal for:** Ultra-low-latency agents (use Cartesia Sonic 3.6 at <90ms TTFA); high-throughput batch generation (use SIMBA 3.2 or Kokoro); offline/on-device deployment (use NeuTTS Air)

## Speechify SIMBA 3.2 — Top-3 Artificial Analysis Speech Arena at $10/1M Characters (Sept 2026)

Speechify SIMBA 3.2 (July 2026) briefly reached #1 on the Artificial Analysis Speech Arena, then #2 behind Qwen-Audio-3.0-TTS-Plus, and now sits around **#3** (Elo ~1,237) as Cartesia Sonic 3.6 and Inworld Realtime TTS-2 have both risen above it — at $10/1M characters ($6/1M at the Scale tier), still the lowest price among the current global top-5 models by a wide margin.
...
- **Best for:** Cost-sensitive high-volume production TTS needing top-3 quality; enterprises benchmarking ElevenLabs alternatives at 10× lower price
- **Not ideal for:** Ultra-low-latency (<100ms) conversational agents (use Cartesia Sonic 3.6)

## Luna-TTS (VUI Labs) — New Top-5 Entrant, #1 on HuggingFace TTS Arena (NEW Aug 2026)

Luna-TTS, from Chinese voice-AI startup VUI Labs, is a new top-tier entrant that reached #1 on the HuggingFace TTS Arena (beating ElevenLabs and MiniMax in blind listener votes) and landed in the top 5 on the Artificial Analysis Speech Arena within weeks of launch (Elo reports vary by source/date, roughly 1,210–1,230 — public documentation on this model is thinner than for the established providers, so treat exact figures as provisional and check artificialanalysis.ai for the current number). A technical report is published on arXiv (2608.11593).

- **URL:** vuilabs.ai
- **Pricing:** ~$80/1M characters via API — notably more expensive than most top-10 competitors (Cartesia, SIMBA, Qwen) at $10–$28/1M characters
- **Free tier:** Not confirmed in public documentation as of Sept 2026
- **Elo:** ~1,210–1,230 on Artificial Analysis Speech Arena (top 5; figure varies by source and date) — #1 on HuggingFace TTS Arena
- **Best for:** Teams chasing top-5 blind-test quality who don't mind thinner documentation and a premium per-character price
- **Not ideal for:** Cost-sensitive high-volume pipelines (SIMBA 3.2 or Speechmatics are far cheaper); production use requiring mature docs/SLAs (use Cartesia, ElevenLabs, or Deepgram)
...
- **Not ideal for:** Ultra-low-latency real-time agents where Cartesia Sonic 3.6's <90ms TTFA is needed; theatrical emotional delivery (use Hume Octave 2)
...
- **Not ideal for:** Ultra-low-latency real-time agents where Cartesia Sonic 3.6's <90ms TTFA is needed; English-only lightweight deployment (use Kokoro or NeuTTS Air for those)

## Breeze TTS 2 (Breeze Blue) — #1 Open-Weight Model on Artificial Analysis (NEW Aug 25, 2026)

Breeze TTS 2 (Breeze Blue) is now the **#1 open-weight model on the Artificial Analysis Provider Voice arena** (Elo 1,215 — 90 Elo points above the previous open-weight leader, Fish Audio S2 Pro — and #6 overall out of 100+ models, ahead of ElevenLabs Eleven v3). It's a real-time flagship built for interactive media (games, digital companions, narrative storytelling) with dynamic controllability over tone, dramatic pacing, subtext, and emotional intensity, plus low-latency streaming. **Licensing caveat:** the inference code is Apache 2.0, but the model weights themselves are research/non-commercial only — unlike Qwen3-TTS or Zyphra ZONOS2, this is not a free-for-commercial-use open weight release; commercial use requires contacting Breeze Blue directly or using their paid hosted API/subscription.

- **URL:** breezeblue.ai | **HuggingFace:** `BreezeBlue/Breeze-TTS-2` | **GitHub (inference code):** github.com/breezeblue-ai/breeze-tts
- **License:** Apache 2.0 for inference code; weights are research/non-commercial only (commercial use via paid hosted API, contact required)
- **Languages:** 50
- **Elo:** 1,215 on Artificial Analysis Provider Voice arena — **#1 among open-weight models** (Sept 2026), #6 overall
- **Best for:** Game/companion/narrative voice where dynamic emotional and pacing control matters and self-hosting for research is acceptable
- **Not ideal for:** Commercial deployment without contacting Breeze Blue first (use Qwen3-TTS or Zyphra ZONOS2 for permissive commercial open weights instead)
...
- **Not ideal for:** Budget-sensitive high-volume use (use Speechmatics $0.011/1K chars or Kokoro at no cost); theatrical emotional range (use Hume Octave 2); top-tier quality ranking (Cartesia Sonic 3.6, Inworld Realtime TTS-2, SIMBA 3.2, Qwen-Audio-3.0-TTS-Plus, and Luna-TTS all rank above it as of Sept 2026)
...
| Best blind-test naturalness (cloud) | Cartesia Sonic 3.6 (Elo ~1,280, **#1** Sept 2026), Inworld Realtime TTS-2 (Elo ~1,250, **#2**), Speechify SIMBA 3.2 (Elo ~1,237, **~#3**), Qwen-Audio-3.0-TTS-Plus (Elo ~1,235, **~#4**), Luna-TTS (VUI Labs, Elo ~1,210–1,230, top 5, new) |
...
| Real-time voice agent / chatbot | Cartesia Sonic 3.6 (<90ms TTFA, now #1 on quality too), Smallest.ai Lightning V3.1 (<100ms), or Inworld Realtime TTS-2 (<200ms, closed-loop, 100+ langs, now #2 overall / #1 Controlled Voice arena) |
...
| Top quality at minimum cost | Speechify SIMBA 3.2 (**top-3 globally** Sept 2026, Elo ~1,237, $10/1M chars; $6/1M at Scale tier) — best price/quality by a wide margin; Cartesia Sonic 3.6 and Inworld Realtime TTS-2 now rank above it but cost more or aren't per-character priced the same way |
...
| Highest-ranked cloud TTS by Elo, real-time streaming | Cartesia Sonic 3.6 (cartesia.ai, Elo ~1,280, **#1** Sept 2026, <90ms latency, free tier); Inworld Realtime TTS-2 (Elo ~1,250, **#2**); Speechify SIMBA 3.2 ($10/1M chars, Elo ~1,237, **~#3**); Qwen-Audio-3.0-TTS-Plus (Alibaba DashScope, Elo ~1,235, **~#4**) |
| Best open-weight model by Elo (self-hostable, research use) | Breeze TTS 2 (Breeze Blue, Elo 1,215, **#1 open-weight**, #6 overall, Sept 2026 — weights are non-commercial only) |
