...
Adds realistic motion to a static photograph (person moves, fabric flows, hair blows). Figma acquired Weavy in October 2025 and relaunched it as Figma Weave (April 2026); node-based canvas lets you chain multiple AI models (image → video → effects) in one workflow, with 20+ workflow templates including image-to-video. Legacy weavy.ai URL still works and redirects/still serves the product.
