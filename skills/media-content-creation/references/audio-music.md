...
- **Studio 2.0 (NEW, August 13, 2026):** Major DAW overhaul, Premier-only for now. Adds MIDI support (import/record/edit MIDI on the timeline, play via MIDI controller or typing keyboard — MIDI clips can even be used as a prompt for new AI-generated audio), a Chat Bar (generate instruments/vocals, build custom plugins and synth presets, ask for help conversationally), studio-grade audio effects (sidechain compression, convolution reverb), a built-in wavetable synth, and Musical Typing with an arpeggiator/chord mode.
...
- **Download caps (effective September 3, 2026):** New policy replaces the prior "no free downloads" rule — Free: 7 lifetime downloads, personal use only, cannot be monetized (no Spotify/YouTube monetization/sync). Pro: 20 downloads/month. Premier: 60 downloads/month. Commercial exploitation of any output now explicitly requires a permitted download through an approved Suno channel under the new Terms. Suno also announced (Aug 6, 2026) that audio watermarking and fingerprinting are coming "in the weeks" after the announcement.
- **Model retirement (pending 2026):** Following a November 2025 settlement with Warner Music Group, Suno committed to retiring all current models (trained on unlicensed audio) once new licensed-catalog models launch. As of August 2026 the transition has not yet occurred — current models remain active — but when it does, all generations made with the old models will eventually become inaccessible. **UMG/Sony (U.S.) litigation:** the expected July 2026 fair-use ruling did not happen — an amended June 30, 2026 scheduling order pushed fact discovery to September 30, 2026 and dispositive motions to April 2027, so a merits decision is now a 2027-earliest event. **GEMA v. Suno (Germany):** resolved — a Munich court ruled against Suno on July 31, 2026, finding it violated copyright on six GEMA-represented songs (training-data reproduction, in-model memorization, and output infringement); Suno owes damages (amount TBD) and says it will evaluate an appeal. Users who want to preserve old generations should download them now on a paid plan.
