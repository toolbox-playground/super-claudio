...
- URL: suno.com
- **Latest model:** v6 (launched September 9, 2026) — Suno's first fully licensed model, built with Warner Music Group, BMG, and Believe; "As v6 rolls out, we will retire our previous models and move Suno entirely onto the v6 generation" per Suno's own announcement — the retirement flagged as pending in earlier 2026 coverage has now happened. Existing songs made on old models remain playable. Previous model: v5.5 (March 2026) — Voices (clone your singing voice), Custom Models (tune on your catalog), My Taste (personalized generation)
...
- **Model retirement (happened Sept 2026):** Following the November 2025 settlement with Warner Music Group, Suno launched v6 on September 9, 2026 (built with Warner, BMG, and Believe) and is retiring all prior unlicensed-data models in favor of it. Users who want to preserve old generations should download them now on a paid plan. **New litigation:** the Warner deal did not cover UMG or Sony — on September 18, 2026, UMG and Sony Music filed a *second* copyright suit against Suno in Boston federal court, naming 60,202 recordings (theoretical max ~$9B in statutory damages), just days after the licensed v6 launch. The earlier GEMA v. Suno case in Germany remains active as well. Treat Suno's licensing status as unresolved despite the Warner/BMG/Believe deal.
...
Mureka V9.5 (Kunlun Tech, released August 31, 2026) is the go-to for developers and technical producers: native API, stem separation, and MIDI export alongside full song generation. V9.5 adds "reflective reasoning" / agentic creation on top of V8, and per multiple third-party blind-vote leaderboards now tops rankings for vocals and instrumentals among tools offering MIDI export.
...
> **Note:** As of September 2026, Udio's licensed platform is still a walled garden — audio/video/stem downloads remain disabled (confirmed unchanged since the Oct 2025 UMG deal; the one-time 48-hour download window from Nov 2025 is long closed, and no re-enable date has been announced). For any use case where you need to keep or use the generated audio, use Suno v6 (paid — its first fully licensed model, see above), ElevenLabs Music v2 (free for personal; Starter plan+ for commercial), Google Flow Music / flowmusic.app (free, Google), or a royalty-free library instead.
>
> **ElevenLabs × UMG (Sept 10, 2026):** ElevenLabs signed a multi-year licensing deal with Universal Music Group to build a fan remix/mashup platform using licensed UMG catalog music. This is separate from ElevenLabs' existing Music API / ElevenMusic (Music v2, above) — no platform name, pricing, or launch date announced yet. Worth checking elevenlabs.io for an announcement if you need licensed-catalog remix capability.
