...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.1 (released Apr 30 2026, became default Jun 11 2026) — 5× faster than v7, native 2K resolution, improved text rendering, HD mode now affordable as default; **v8.2 became the default version July 24, 2026**, introducing a new **Edit Model** that replaces Omni Reference, Character Reference, and the old Retexture tool with one unified tool — instructed edits, generation from up to 4 reference images, and Editor-based inpainting/outpainting; bolder aesthetics and improved Personalization (old Retexture still reachable by switching to v7 or earlier) | Paid (no free tier) |
...
| **GPT Image 2.5** (Flare / Sunburst) | ChatGPT / API | Complex instructions, text in images, 4K resolution; launched Sept 8, 2026 as two API models — **Flare** (speed-first, ~50% lower latency than GPT Image 2) and **Sunburst** (precision-first, slower, more controlled multi-turn edits); adds a Sketch tool, Templates, and comment-based editing in ChatGPT; predecessor GPT Image 2 (launched April 21, 2026) had topped Artificial Analysis's Text-to-Image Arena through mid-2026 before newer models (MAI-Image-2.6, Reve 2.1) closed the gap | Limited (ChatGPT Plus); API models `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst`, token-based pricing ($5/1M text-input tokens, $8/1M image-input tokens, $30/1M image-output tokens) |
| **Grok Imagine Image 2.0** (xAI) | grok.com/imagine, Grok iOS/Android apps | Fast generation with built-in region editing — magic-wand tool edits only the area you point at, one-click background removal (transparent export), multi-reference input (up to 5 images) for compositing without manual stitching; shipped as "Quality Mode" Aug 7, 2026; xAI claims #2 on Arena's text-to-image and image-edit leaderboards (behind GPT Image) | Included with Grok (free tier available at grok.com); no public API yet — xAI says it's "coming soon" |
...
| **MAI-Image-2.6 / Flash** (Microsoft) | microsoft.ai (MAI Playground), Microsoft Foundry (public preview since Sept 4, 2026), Azure AI Foundry | High-quality generation and precise, "surgical" localized editing (targeted object edits, layout changes, text updates, artifact cleanup) with consistency preserved across iterations; **#2 on Arena for both text-to-image and image editing** — ahead of Google, Meta, and xAI's entries — and **#1 image editing on Artificial Analysis**; +79 Elo over MAI-Image-2.5 (previously #2 image editing / #3 text-to-image, launched June 2, 2026); adds multi-image reference editing, web grounding, dynamic aspect ratios; Flash variant tuned for latency-sensitive, high-throughput production | No free tier; 2.6 pricing not yet published at Foundry public-preview launch (predecessor MAI-Image-2.5: $47/1M image output tokens standard, $33/1M Flash; text input $5/1M standard, $1.75/1M Flash) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; **2.1 (released July 9, 2026, about a month after 2.0)** sharpened prompt/world-knowledge understanding and foreign-text rendering, +17 Elo over 2.0, narrowing the gap with GPT Image 2; still the top-ranked independent-lab model on Arena and Design Arena | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
