...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; became the default model July 24, 2026 (preview flag opened June 25, 2026) — aesthetic refinements, stronger Personalization, better Sref/moodboard consistency, Big Batch Draft Mode with `--sref random` for 24× faster style exploration; v8.1 (Apr 30 2026): 5× faster than v7, native 2K resolution, improved text rendering | Paid (no free tier) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; released July 9, 2026 with sharper prompt understanding and foreign-text rendering; **#2 on Artificial Analysis Text-to-Image Arena (Elo 1299)**, behind only GPT Image 2 | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
| **FLUX 3** (Black Forest Labs) — Early Access | bfl.ai | Unified multimodal flow model spanning image, video, audio, and robot-action prediction in one architecture; announced July 23, 2026; FLUX 3 Video generates up to 20s clips with native audio; image benchmarks still preliminary/mid-training; open-weight FLUX 3 Dev planned later | Early access only (apply via BFL); not yet priced |
| **FLUX.2 [max]** (Black Forest Labs) | bfl.ai, fal.ai, openrouter.ai | Highest-quality *production* FLUX model (FLUX 3 above is early access only); Grounded Generation (retrieves real-time web context during image gen); up to 10 reference images for consistent products/characters/styles; #2 on Artificial Analysis Image Generation (behind Nano Banana Pro); open-source [dev] variant (32B params) released alongside | API credits ($0.07/mp first output mp, $0.03/mp subsequent; [dev]: open-weights, non-commercial) |
