...
| **GPT Image 2.5** (OpenAI) | ChatGPT / API | Complex instructions, text in images (99% accuracy), sharper detail, up to 4K, up to 16 reference images, new `xhigh`/`max` quality tiers, ~50% lower latency, Sketch-to-image; released Sept 8, 2026, supersedes GPT Image 2; **#1 on Artificial Analysis Text-to-Image Arena — widest #1–#2 Elo gap on record** | Limited (ChatGPT free); ChatGPT Plus for full access |
...
| **MAI-Image-2.6 / Flash** (Microsoft) | microsoft.ai (MAI Playground), Microsoft Foundry, Copilot, Azure AI Foundry | High-quality generation and precise localized editing; announced Aug 10, 2026, public preview opened Sept 4, 2026; **#2 on Arena** — +79 Elo and +91 text-rendering Elo over MAI-Image-2.5; strong product imagery, text rendering, and prompt adherence; Flash variant for lower cost; integrating into PowerPoint (generation) and OneDrive (photo editing) | Free via Copilot (consumer); API pricing similar to 2.5 tier |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; iterates on visual intelligence/world knowledge vs 2.0; launched July 9, 2026 | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
| **Grok Imagine Image 2.0** (xAI) | grok.com, x.ai/api | Partial/masked editing, segmentation, background transparency; launched Aug 7, 2026; briefly #2 on Arena (text-to-image and editing), later ~#3 as rivals shipped | Free tier effectively removed (~2 images/day on free accounts); full access needs SuperGrok ($30/mo); API: $0.04–$0.08/image |
...
| **Riverflow 2.5 Pro** (Sourceful) | openrouter.ai/sourceful/riverflow-v2-pro, runware.ai, wavespeed.ai | Photorealistic product mockups, packaging, ads, brand assets; announced June 18, 2026 (upgrade from 2.0 Pro) — **#1 across Design Arena's Image, Graphic Design, and Image Editing categories simultaneously**; 4K output, transparent background support, agentic precision for commercial imagery | API credits ($0.15/image 2K, $0.33/image 4K; no direct consumer site — API only) |
...
Google deprecated all Imagen model endpoints in two waves. Imagen 3 endpoints shut down June 24–30, 2026. Imagen 4 endpoints (`imagen-4.0-generate-001`, `imagen-4.0-ultra-generate-001`, `imagen-4.0-fast-generate-001`) **shut down as scheduled on August 17, 2026 — this has now happened**; calls fail with a hard error (no grace period). Migrate to Gemini Image models ("Nano Banana"): the recommended path is `gemini-3.1-flash-image` — note this is not a like-for-like swap, the API shape changed from `generate_images()` to `generate_content()`, so client code needs updating, not just the model name. Firebase AI Logic and Vertex AI users on the old endpoints are now hard-broken. Google AI Studio provides a migration guide at firebase.google.com/docs/ai-logic/imagen-models-migration.

## FLUX 3 — Announced, Image Variant Not Yet Public (Jul 2026)

Black Forest Labs announced FLUX 3 on July 23, 2026 as a unified multimodal model spanning image, 20-second video+audio, and robotics. Only FLUX 3 Video and FLUX 3 Action are in early access; as of this update, **FLUX 3 Image has no public endpoint, model ID, or pricing** — it remains gated. Do not recommend it yet; continue using FLUX.2 [max]/[pro] for image generation until FLUX 3 Image ships.
