...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 (GA/default since Jul 24 2026, retiring the v8.0 branch) — new unified Edit Model replaces Omni Reference, Character Reference, and Retexture; bolder/more sophisticated default aesthetic, improved personalization-profile taste matching; v8.1 (Apr 30 2026, default Jun 11–Jul 24 2026) added 5× speed vs v7, native 2K resolution, improved text rendering | Paid (no free tier) |
...
| **Meta Muse Image** (Meta Superintelligence Labs) | meta.ai, Meta AI app, Instagram Stories (US), WhatsApp (limited countries); developer API added ~Sept 2026 via dev.meta.ai (Meta Model API) and fal.ai | Social-native image generation — text-to-image, multi-photo blending, @mention of public Instagram accounts for likeness reference, sketch-and-instruction editing, 30+ Instagram Story effects; agentic tool use (web search + coding invoked during generation); self-refinement at inference time (test-time compute scaling before delivering the final image); Content Seal invisible watermark; launched July 7, 2026; ranked among the top text-to-image and image-edit arenas as of mid-2026; **developer API now live** (generate, edit, and compose endpoints) — previously consumer-only | Free (consumer; subscription for power users via Meta AI subscription plans); API (fal.ai / Meta Model API): ~$0.01/image |
...
| **FLUX 3** (Black Forest Labs) | bfl.ai (early access) | **Announced Jul 23, 2026 — not yet the recommended pick.** Unified multimodal flow model spanning image, video (up to 20s with native audio), and robot-action prediction; Video and Action available in early access via API/select partners; the Image tier had not shipped as of the announcement and still needs a status re-check before recommending over FLUX.2 | Early access; no public pricing yet |
| **FLUX.2 [max]** (Black Forest Labs) | bfl.ai, fal.ai, openrouter.ai | Highest-quality FLUX model generally available today; Grounded Generation (retrieves real-time web context during image gen); up to 10 reference images for consistent products/characters/styles; #2 on Artificial Analysis Image Generation (behind Nano Banana Pro); open-source [dev] variant (32B params) released alongside | API credits ($0.07/mp first output mp, $0.03/mp subsequent; [dev]: open-weights, non-commercial) |
...
- **Developer API launched ~September 2026** via dev.meta.ai (Meta Model API) and fal.ai — generate, edit, and compose endpoints; priced for production volume at ~$0.01/image
- Content Seal invisible watermark embedded in all outputs
- Best for: social-first creators already in the Meta ecosystem; campaigns tapping Instagram social graph for reference imagery; Instagram Story effects workflows; now also viable for developers needing programmatic access
- Not ideal for: high-volume batch generation at the lowest cost (compare FLUX.2 [klein] or Nano Banana 2 Lite); print/4K quality output (use FLUX.2 [max] or GPT Image 2 for those)
