...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; 2.1 (released July 9, 2026) improves prompt adherence, world knowledge, spatial reasoning, and non-English text rendering over 2.0; **#2 on Artificial Analysis Text-to-Image Arena (Elo 1299) and #1 on the Image Editing Arena (Elo 1259)**, both behind only GPT Image 2 on the text-to-image side | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
## FLUX 3 — Next-Gen Multimodal Model (Preview, Aug 2026)

Black Forest Labs unveiled FLUX 3 on July 23, 2026: a single multimodal model spanning image, video, audio, and robot-action prediction. FLUX 3 Video reached general availability on August 4, 2026 — 20-second clips with native, synced audio (not dubbed on afterward), available via fal.ai and ComfyUI Partner Nodes (see `image-motion.md` for the image-to-video workflow). **Standalone FLUX 3 image generation has not launched yet** — BFL says pricing, model IDs, and API access are coming "in the coming weeks." Keep using FLUX.2 [max]/[pro] (still current, in the table above) for text-to-image and photorealistic work until FLUX 3 Image ships; this reference will be updated once it does.
