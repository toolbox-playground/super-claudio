...
| **GPT Image 2** | ChatGPT / API | Complex instructions, text in images (99% accuracy), 4K resolution, 2× faster than predecessor; launched April 21, 2026; **#1 on Artificial Analysis Text-to-Image Arena (Elo 1381, Aug 2026; was ~1339 in July)** — gap over #2 widening | Limited (ChatGPT Plus) |
| **MAI-Image-2.6** (Microsoft) | microsoft.ai (MAI Playground), Microsoft Foundry (Private Preview) | Successor to MAI-Image-2.5, launched Aug 10, 2026; **#2 on Artificial Analysis Text-to-Image Arena (Elo 1336 ±11)** — +79 over 2.5, biggest single-category jump in text rendering (+91) | No permanent free tier (test free on Arena / MAI Playground) |
| **Grok Imagine Image 2.0** (xAI) | grok.com/imagine, Grok iOS/Android ("Quality Mode") | Editing-first model — magic-wand regional edits, segmentation, background removal, multi-reference input (up to 5 images), strong text/layout rendering; released Aug 7, 2026; xAI claims #2 on Arena text-to-image and image-edit leaderboards (self-reported) | Free (Grok app); API "coming soon", no public pricing yet |
...
| **MAI-Image-2.5 / Flash** (Microsoft) | microsoft.ai (MAI Playground), openrouter.ai/microsoft/mai-image-2.5, Azure AI Foundry | High-quality generation and precise localized editing (launched June 2, 2026 at Microsoft Build); **superseded by MAI-Image-2.6 (Aug 10, 2026, above) as Microsoft's flagship** — 2.5/Flash still available via API; strong product imagery, text rendering, and prompt adherence; Flash variant for 2.7× lower cost; integrating into PowerPoint (generation) and OneDrive (photo editing) | No free tier (API: $47/1M image output tokens standard; $33/1M Flash; text input: $5/1M standard, $1.75/1M Flash) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; **2.1 (Jul 9, 2026) improved prompt understanding, world knowledge, and foreign-text rendering**; ranked #2 on Image Arena shortly after launch (Elo ~1306–1321), likely displaced to #3 by MAI-Image-2.6 | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
| **Google Imagen 4** **(DEPRECATED — Imagen 4 endpoints confirmed shut down August 17, 2026 as scheduled, no grace period; Imagen 3 already shut down June 24–30, 2026; migrate to Nano Banana / Gemini Image models)** | — | — | — |
...
Google deprecated all Imagen model endpoints in two waves — both now complete. Imagen 3 endpoints shut down June 24–30, 2026. Imagen 4 endpoints (`imagen-4.0-generate-001`, `imagen-4.0-ultra-generate-001`, `imagen-4.0-fast-generate-001`) shut down **August 17, 2026 as scheduled, confirmed hard-erroring with no grace period**. Migrate to Gemini Image models ("Nano Banana"): swap the model name from `imagen-*` to the corresponding `gemini-*-image` model in your API calls. Firebase AI Logic and Vertex AI users needed to update before August 17 or requests now fail. Google AI Studio provides a migration guide at firebase.google.com/docs/ai-logic/imagen-models-migration.

## FLUX 3 — Announced, Not Yet Released (Watch)

Black Forest Labs announced the FLUX 3 family (image + video + audio + robot action, multimodal) July 26–28, 2026. **FLUX 3 Video** went GA August 5, 2026 (1080p, up to 20s clips, synced audio), but **FLUX 3 Image itself has not shipped** as of mid-August 2026 — no model IDs or pricing published. Keep using FLUX.2 [max]/[pro]/[klein] (above) until FLUX 3 Image reaches GA.
