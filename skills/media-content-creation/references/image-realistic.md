...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 became the **default model around July 24, 2026** (no longer gated behind `--preview`) — aesthetic refinements over v8.1, better Sref/moodboard consistency, Big Batch Draft Mode with `--sref random` for 24× faster style exploration; v8.1 (Apr 30 2026) was 5× faster than v7 with native 2K resolution and improved text rendering | Paid (no free tier) |
...
| **MAI-Image-2.5 / Flash / Pro** (Microsoft) | microsoft.ai (MAI Playground), openrouter.ai/microsoft/mai-image-2.5, Azure AI Foundry | High-quality generation and precise localized editing; #2 image editing, #3 text-to-image on Arena (launched June 2, 2026 at Microsoft Build); +75 pts over MAI-Image-2, biggest gains in Text Rendering (+107) and Cartoon/Anime/Fantasy (+90); strong product imagery, text rendering, and prompt adherence; Flash variant for 2.7× lower cost; **Pro variant in public preview since July 23, 2026** — Microsoft's highest-fidelity image model to date; Bing Image Creator now runs entirely on the MAI-Image-2.5 family | No free tier (API: $47/1M image output tokens standard; $33/1M Flash; Pro: $5/1M text input, $8/1M image input, $106/1M image output tokens) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; **released July 9, 2026 — #2 on Artificial Analysis Text-to-Image Arena (Elo 1306, +36 vs 2.0), still the top-ranked 4K-native model** | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
| **FLUX 3** (Black Forest Labs) | bfl.ai (early access) | **Announced July 23, 2026 — not yet fully shipped.** BFL's first unified architecture jointly trained on image, video, and audio, extendable to robot actions (FLUX-mimic, tested with Audi); Video/Action variants in early-access API to select partners now; **Image variant "expected in coming weeks" (not yet live as of Aug 2, 2026)** — do not recommend over FLUX.2 until it ships; open-weight FLUX 3 Dev planned for later in 2026; text-to-video up to 20s with native synced audio | Early access only; pricing TBA |
