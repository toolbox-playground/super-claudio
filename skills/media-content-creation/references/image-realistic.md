...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 became the default model on Jul 24, 2026 (up from v8.1, Apr 30 2026) — aesthetic refinements (bolder, more "creative and edgy" default style), much stronger personalization (larger training image pool, better use of rating history), Big Batch Draft Mode with `--sref random` for fast style exploration; v9 is in internal preview/testing as of Sept 2026 with no announced release date | Paid (no free tier) |
...
| **GPT Image 2.5** | ChatGPT / API | Complex instructions, text in images, 4K resolution; released Sept 8, 2026 as "ChatGPT Images 2.5" — adds Sketch (draw a reference directly in ChatGPT via `@Sketch`, with Poster/Merch templates), more natural lighting/texture, better preservation of subjects from reference photos across multi-turn edits, and up to 50% lower generation latency than Images 2.0; API ships two variants, Flare (fast default) and Sunburst (precision editing); **#1 and #2 on Artificial Analysis Text-to-Image Arena** (Flare Elo 1188, Sunburst Elo 1182, Sept 2026); prior GPT Image 2 (Apr 21, 2026) still listed separately on the arena at #3 (Elo 1171) | Limited (ChatGPT Plus); API: Flare/Sunburst $8/$30 per 1M input/output tokens |
| **Grok Imagine Image 2.0** (xAI) | grok.com/imagine, Grok iOS/Android app (Quality Mode) | Fast in-app region editing — magic-wand region edits, segmentation, background removal, up to 5 reference images, Smart Resize across 9 aspect ratios, strong typography/layout planning; launched Aug 7, 2026; debuted #2 worldwide on both text-to-image and image-editing arenas (behind only OpenAI's GPT Image models); **#4 on Artificial Analysis Text-to-Image Arena (Elo 1154, Sept 2026)**; dedicated API listed as "coming soon" with no date announced | Free with existing Grok access (no separate charge); no public API yet |
...
| **MAI-Image-2.6 / Flash** (Microsoft) | microsoft.ai (MAI Playground), Microsoft Foundry (Public Preview), Azure AI Foundry | High-quality generation and precise localized editing; launched Sept 4, 2026, superseding MAI-Image-2.5; +79 Elo over 2.5 across every Arena text-to-image category (text rendering alone +91 Elo); debuted #2 on Arena text-to-image, ahead of Google, Meta, and xAI models; multi-image reference editing, web grounding, dynamic aspect ratios; Flash variant generates 2.8× faster than GPT-Image-2-Medium at 72% greater compute efficiency | No confirmed free tier; Public Preview on Microsoft Foundry — per-token API pricing for 2.6 not yet published (2.5 pricing was $47/1M image output tokens standard, $33/1M Flash) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first native 4K generation — plans an editable, addressable layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; stronger spatial planning and multilingual text rendering than 2.0; launched July 9, 2026; per-operation editing priced at $0.01 | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
| **Qwen-Image-3.0** (Alibaba) | chat.qwen.ai, Alibaba Cloud Model Studio (DashScope API) | Dense, text-heavy layouts — newspaper-style pages, infographics, storyboards, exam sheets in a single pass; accepts prompts up to ~4,500 tokens; renders legible text as small as ~10px, math formulas, and 12 languages in one pass; released July 21, 2026 (invite-only), opened to all Qwen users Aug 5, 2026; shipped without a technical report, model card, or published benchmark scores (unlike 1.0/2.0, which shipped same-day reports) | Free tier (Qwen app); API (DashScope): ~$0.025/image standard; ~$0.035 (1K) to ~$0.07 (2K) per image on Pro tier |
...
## Grok Imagine Image 2.0 — xAI's Fast In-App Editor (August 2026)

xAI shipped Grok Imagine Image 2.0 on August 7, 2026 as the "Quality Mode" inside Grok's Imagine tool — no separate product or plan, just an upgrade for existing Grok users. It debuted #2 in the world on both the text-to-image and image-editing arenas (behind only OpenAI's GPT Image models), later settling around #4 on Artificial Analysis's Text-to-Image Arena as GPT Image 2.5 and MAI-Image-2.6 shipped in September.

- Editing toolset: magic-wand region selection, segmentation, one-click background removal, Smart Resize across 9 aspect ratios
- Accepts up to 5 reference images for blending and consistency
- Plans typography and layout the way a designer would; preserves elements across generations and edits
- **Access:** grok.com/imagine or the Grok iOS/Android app, included with existing Grok access at no extra charge
- **No public API yet** — xAI has said a dedicated Imagine Image 2.0 API is "coming soon" with no date given
- Best for: users already inside the Grok/X ecosystem who want fast, free region-level edits; not yet an option for programmatic/batch workflows until the API ships
