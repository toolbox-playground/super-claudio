...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 became the default version July 24, 2026 (aesthetic refinements, better Sref/moodboard consistency, Big Batch Draft Mode); new **Edit Model** (Aug 27, 2026) replaces Omni Reference/Character Reference/Retexture with unified instruction-based editing, inpainting/outpainting, up to 4 reference images | Paid (no free tier) |
...
| **GPT Image 2** | ChatGPT / API | Complex instructions, text in images (99% accuracy), 4K resolution, 2× faster than predecessor; launched April 21, 2026; **#1 on Artificial Analysis Text-to-Image Arena (Elo ~1370, Sept 2026)** — lead over #2 MAI-Image-2.6 narrowed to ~21 Elo since July; also competes at the top of the AA Image Editing Leaderboard | Limited (ChatGPT Plus) |
| **Meta Muse Image** (Meta Superintelligence Labs) | meta.ai, Meta AI app, Instagram Stories (US), WhatsApp (limited countries) | Social-native image generation — text-to-image, multi-photo blending, @mention of public Instagram accounts for likeness reference, sketch-and-instruction editing, 30+ Instagram Story effects; agentic tool use (web search + coding invoked during generation); self-refinement at inference time (test-time compute scaling before delivering the final image); Content Seal invisible watermark; launched July 7, 2026; **roughly #4–#5 on Artificial Analysis Text-to-Image ranking (Sept 2026)** — MAI-Image-2.6 and Reve 2.1 landed since July and displaced its July #2 spot (may still show #2 on Meta's own cited Arena.ai numbers); no developer API at launch (Meta evaluating external access) | Free (consumer; subscription for power users via Meta AI subscription plans) |
| **MAI-Image-2.6 / Flash** (Microsoft) | microsoft.ai (MAI Playground), openrouter.ai/microsoft/mai-image-2.6, Azure AI Foundry | High-quality generation and precise localized editing; launched Aug 10, 2026, +79 Elo over MAI-Image-2.5; **#2 on Artificial Analysis Text-to-Image Arena (behind only GPT Image 2)**, competes at the top of the Image Editing Leaderboard; strong product imagery, text rendering, and prompt adherence; Flash variant for lower cost; integrating into PowerPoint (generation) and OneDrive (photo editing) | No free tier (API pricing similar to prior 2.5 tier — check microsoft.ai for current rates) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing (from $0.01/edit), multi-reference composition; native 4K output; 2.1 (July 9, 2026) improved prompt understanding, world knowledge, spatial reasoning, and non-English text rendering vs 2.0; retains #2 overall on Image Arena, still top independent-lab / #1 4K model | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
| **Riverflow 2.5** (Sourceful) | openrouter.ai/sourceful/riverflow-v2-pro, runware.ai, wavespeed.ai | Photorealistic product mockups, packaging, ads, brand assets — 2.5 (Apr 2026) added reasoning-driven editing and Font Control on top of 2.0 Pro's #1 Artificial Analysis Image Generation leaderboard debut (Feb 2026); 4K output, transparent background support, agentic precision for commercial imagery | API credits ($0.15/image 2K, $0.33/image 4K; no direct consumer site — API only) |
| **FLUX.2 [max]** (Black Forest Labs) | bfl.ai, fal.ai, openrouter.ai | Highest-quality FLUX.2 model; Grounded Generation (retrieves real-time web context during image gen); up to 10 reference images for consistent products/characters/styles; **no longer top-tier — ~Elo 1190 on Artificial Analysis (Sept 2026), behind GPT Image 2, MAI-Image-2.6, Reve 2.1, and Nano Banana models** since their launches; open-source [dev] variant (32B params) released alongside | API credits ($0.07/mp first output mp, $0.03/mp subsequent; [dev]: open-weights, non-commercial) |
| **FLUX 3** (Black Forest Labs) | bfl.ai | Unified multimodal model (image + video + audio, "Self-Flow" architecture); video+audio generation (up to 20s with synchronized audio) went GA Aug 4, 2026 via BFL API and select partners; **still-image generation was still rolling out as of early Sept 2026 — not yet a drop-in still-image tool** | API (video GA); image gen TBA — check bfl.ai for current status |
...
| **Stable Diffusion 4** | Local / Automatic1111 | Full control, local, free; SD4 (Apr 2026, DiT architecture) added native 4096×4096 output and a dedicated text-rendering module vs. SD3 | Yes (local) |
