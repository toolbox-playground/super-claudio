...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 became the default model July 24, 2026 (superseding v8.1) — aesthetic refinements, better Sref/moodboard consistency, Big Batch Draft Mode with `--sref random` for 24× faster style exploration; **V8.2 Edit Model** opened public testing Aug 27, 2026, replacing Omni Reference/Character Reference/Retexture with up to 4 image references | Paid (no free tier) |
...
| **GPT Image 2.5 (Flare / Sunburst)** | ChatGPT / API | Supersedes GPT Image 2 (launched Sept 8, 2026); Flare = speed tier (default, 50% lower latency), Sunburst = precision/editing tier; new Sketch-to-image feature in ChatGPT; **#1 on Artificial Analysis Text-to-Image and Image-Editing Arenas (Elo ~1188 Flare / ~1176 Sunburst editing, Sept 2026)** | Limited (ChatGPT Plus); API: $8/1M image-input tokens, $30/1M image-output tokens, $5/1M text-input tokens |
...
| **MAI-Image-2.6 / Flash** (Microsoft) | microsoft.ai (MAI Playground), openrouter.ai/microsoft/mai-image-2.6, Azure AI Foundry | Supersedes MAI-Image-2.5 (released ~Sept 4, 2026); +79 Elo over 2.5, biggest gain in text rendering (+91); #2 on Arena text-to-image, ahead of Google/Meta/xAI; Flash variant 2.8× faster than GPT Image 2 Medium; multi-image reference editing, web grounding, dynamic aspect ratios | Public Preview on Microsoft Foundry (2.6-specific per-token pricing not yet published) |
...
| **Grok Imagine Image 2.0** (xAI) | grok.com/imagine, Grok iOS/Android apps | GA since Aug 7, 2026; strong instruction following, typography/layout control; precise regional edits (magic wand, segmentation, background removal with transparency); multi-reference input (up to 5 images); Smart Resize; #2 on Arena text-to-image and image-edit (behind GPT Image 2.5) | Included in Grok subscription (Quality Mode; free tier is limited/unstable since xAI cut free image gen in March 2026); no public API yet | grok.com/imagine |
| **Qwen-Image-2.1** (Alibaba) | HuggingFace (Qwen), ModelScope, GitHub; day-zero support in ComfyUI, Diffusers, vLLM-Omni, SGLang | Open-sourced Sept 20, 2026; 7B-param unified text-to-image + editing model; native RGBA/transparency output; accepts up to 10 reference images; local/regional edits in one checkpoint; ranks #1 among open-source image models on public evals (ahead of Nano Banana 2) | Weights free to download; **Qwen Research License — commercial use barred**, requires separate license grant | huggingface.co/Qwen |
