...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; became the default model **July 24, 2026** (up from v8.1, default since Jun 11 2026); replaces Omni Reference, Character Reference, and Retexture with a single unified "Edit Model"; sharper aesthetics and personalization, Big Batch Draft Mode with `--sref random` for 24× faster style exploration | Paid (no free tier) |
...
| **GPT Image 2** | ChatGPT / API | Complex instructions, text in images (99% accuracy), 4K resolution, 2× faster than predecessor; launched April 21, 2026; **#1 on Artificial Analysis Text-to-Image Arena (Elo ~1339)** — still #1 as of Sept 2026, reportedly the largest first-to-second Elo gap ever recorded on the arena | Limited (ChatGPT Plus) |
| **MAI-Image-2.6 / Flash** (Microsoft) | microsoft.ai (MAI Playground), openrouter.ai/microsoft/mai-image-2.6, Azure AI Foundry | Launched **Sept 4, 2026**, replacing MAI-Image-2.5; debuted **#2 on the Arena text-to-image leaderboard** (79 Elo pts above 2.5), ahead of Meta Muse and Google — knocked Meta Muse Image off #2; strong product imagery, text rendering, prompt adherence, localized editing; Flash variant for faster/cheaper generation; integrating into PowerPoint (generation) and OneDrive (photo editing) | No free tier (API pricing per token, see microsoft.ai) |
| **Meta Muse Image** (Meta Superintelligence Labs) | meta.ai, Meta AI app, Instagram Stories (US), WhatsApp (limited countries) | Social-native image generation — text-to-image, multi-photo blending, @mention of public Instagram accounts for likeness reference, sketch-and-instruction editing, 30+ Instagram Story effects; agentic tool use (web search + coding invoked during generation); self-refinement at inference time (test-time compute scaling before delivering the final image); Content Seal invisible watermark; launched July 7, 2026; held #2 on Arena.ai Text-to-Image Arena through Aug 2026 (Elo 1280) — **displaced from #2 by MAI-Image-2.6 in Sept 2026**; no developer API at launch (Meta evaluating external access) | Free (consumer; subscription for power users via Meta AI subscription plans) |
...
| **Seedream 5.0 Lite** (ByteDance) | seed.bytedance.com, pixverse.ai | Launched Aug 13, 2026 — cheaper/lighter tier of Seedream 5.0 for high-volume, lower-cost generation when full 5.0/5.0 Pro quality isn't needed | Limited free (via PixVerse) |
...
| **FLUX 3** (Black Forest Labs) | bfl.ai, fal.ai | New multimodal flow model spanning image/video/audio/robot-action, not just stills; launched July 23–Aug 4, 2026; video+audio generation mode (20s clips) shipped in early access Aug 4, 2026; **standalone image-generation mode was still rolling out ("coming weeks") as of early Sept 2026** — verify current status before recommending over FLUX.2 for pure image work | API credits (see bfl.ai for current pricing) |
| **FLUX.2 [max]** (Black Forest Labs) | bfl.ai, fal.ai, openrouter.ai | Highest-quality FLUX.2 model; Grounded Generation (retrieves real-time web context during image gen); up to 10 reference images for consistent products/characters/styles; #2 on Artificial Analysis Image Generation (behind Nano Banana Pro); open-source [dev] variant (32B params) released alongside; still the recommended pick for pure image generation while FLUX 3's image mode finishes rolling out | API credits ($0.07/mp first output mp, $0.03/mp subsequent; [dev]: open-weights, non-commercial) |
