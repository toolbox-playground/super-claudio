...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 became the default Jul 24 2026 (was in preview via `--preview` since Jun 25 2026), replacing v8.1 (default Jun 11–Jul 23 2026); bolder, more sophisticated aesthetics, dramatically reduced low-quality outputs, improved Personalization/Sref-moodboard consistency, Big Batch Draft Mode with `--sref random` for 24× faster style exploration | Paid (no free tier) |
...
| **GPT Image 2** | ChatGPT / API | Complex instructions, text in images (99% accuracy), 4K resolution, 2× faster than predecessor; launched April 21, 2026; **#1 on Artificial Analysis Text-to-Image Arena (Elo 1369, August 2026)** | Limited (ChatGPT Plus) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; shipped July 9, 2026 (one month after 2.0) — sharper prompt/intent understanding, stronger text rendering, biggest gains in marketing materials, abstract patterns, and people; **#2 on Artificial Analysis Text-to-Image Arena (Elo 1325, behind GPT Image 2) and #2 on Arena.ai (Elo 1306)**; **#1 on Artificial Analysis Image Editing Arena (Elo 1262)** | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
> **Watch for FLUX 3:** Black Forest Labs announced FLUX 3 on July 23, 2026 — a multimodal flow model spanning image, video (up to 20s with native audio), and robot-action prediction in one architecture ("Self-Flow"). Rollout is staged: FLUX 3 Video is in early access (API + private weights to partners); **FLUX 3 Image had not yet reached general availability** as of early August 2026 ("rolling out in the coming weeks"); FLUX 3 Dev (open-weight) is slated for later in 2026. The FLUX.2 rows below remain the current recommendation until FLUX 3 Image ships — re-check via `find-ai-tools` before relying on this note.
