...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 became the default version Jul 24, 2026 (v8.1 was default Jun 11–Jul 23, 2026) — bolder, more sophisticated aesthetics, better personalization, and a new unified Edit Model that replaces Omni Reference, Character Reference, and the Retexture tool; v8.1 remains selectable and is still ~5× faster than v7; native 2K resolution | Paid (no free tier) |
...
| **Nano Banana 2** (Google / Gemini) | gemini.google.com | Product images, lifestyle scenes, ad creatives — default free model (Gemini 3.1 Flash Image); **#4 on Artificial Analysis Text-to-Image Arena (Elo 1321) and #3 on Image Editing Arena (Elo 1311), Sept 2026** | Free (Gemini app) |
| **Nano Banana 2 Lite** (Google / Gemini) | gemini.google.com, aistudio.google.com | Fastest & cheapest Google image model; 2.7× faster than Nano Banana 2 (~4 s); 1K resolution cap; ranked #5 on Artificial Analysis Image Arena (Elo 1261) at its June 30, 2026 launch — highest-ranked free model per dollar; several new models (MAI-Image-2.6, Reve 2.1, Muse Image) have since launched above it | Free (Google AI Studio); API: $0.034/image (batch: $0.017) |
...
| **GPT Image 2** | ChatGPT / API | Complex instructions, text in images (99% accuracy), 4K resolution, 2× faster than predecessor; launched April 21, 2026; **#1 on Artificial Analysis Text-to-Image Arena (Elo 1371, Sept 2026 — up from ~1339 in July) and #2 on Image Editing Arena (Elo 1325)** | Limited (ChatGPT Plus) |
| **Meta Muse Image** (Meta Superintelligence Labs) | meta.ai, Meta AI app, Instagram Stories (US), WhatsApp (limited countries) | Social-native image generation — text-to-image, multi-photo blending, @mention of public Instagram accounts for likeness reference, sketch-and-instruction editing, 30+ Instagram Story effects; agentic tool use (web search + coding invoked during generation); self-refinement at inference time (test-time compute scaling before delivering the final image); Content Seal invisible watermark; launched July 7, 2026 at #2 on Arena.ai (Elo 1280); by Sept 2026 sits around **#5 on the Text-to-Image Arena (Elo ~1312)** as MAI-Image-2.6, Reve 2.1, and Nano Banana 2 climbed above it; no developer API yet (Meta evaluating external access) | Free (consumer; subscription for power users via Meta AI subscription plans) |
| **MAI-Image-2.6** (Microsoft) | microsoft.ai (MAI Playground), openrouter.ai/microsoft/mai-image-2.6, Azure AI Foundry | High-quality generation and precise localized editing; released Aug 10, 2026, superseding MAI-Image-2.5; **#2 on Artificial Analysis Text-to-Image Arena (Elo 1349) and #1 on Image Editing Arena (Elo 1330), Sept 2026** — only GPT Image 2 ranks higher for text-to-image; +79 Elo over 2.5, with the biggest jumps in 3D modeling (+114), portraits (+105), and text rendering (+91); multi-reference-image input, grounding, and adjustable reasoning/output/resolution controls; strong for product imagery and photorealism; rolling out to MAI Playground, Microsoft Foundry, PowerPoint, and OneDrive | No free tier (API pricing carried over from 2.5: ~$47/1M image output tokens standard, $33/1M Flash tier) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; released Jul 9, 2026 (one month after 2.0) with stronger prompt understanding, world knowledge, and foreign-text rendering; **#3 on Artificial Analysis Text-to-Image Arena (Elo 1324, Sept 2026)** — still the top 4K model, trained on far fewer GPUs than rivals | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
| **Seedream 5.0 Pro** (ByteDance) | seed.bytedance.com, byteplus.com, fal.ai | Precision editing + layered output — launched July 8, 2026; splits one render into 10+ transparent PNG layers (auto-fills occluded areas for maskless editing in Figma/Photoshop); pixel-level spatial grounding for interactive regional edits; multilingual text generation in 10+ languages (Chinese, English, French, German, Russian, Japanese, Korean, Spanish, Arabic — includes RTL layouts); realistic portrait textures and multi-person compositing; available via BytePlus API, Doubao/Jimeng consumer apps, fal.ai, and ComfyUI; **#4 on Artificial Analysis Image Editing Arena (Elo 1308, Sept 2026)** | API: $0.075/image ≤2.36MP; $0.150/image >2.36MP; $0.005 per extra input image |
| **Riverflow 2.5 Pro** (Sourceful) | openrouter.ai/sourceful/riverflow-v2.5-pro, riverflow.ai, runware.ai, wavespeed.ai | Photorealistic product mockups, packaging, ads, brand assets — released June 2026, superseding Riverflow 2.0 (which had taken #1 on Artificial Analysis Text-to-Image and Image Editing leaderboards in Feb 2026); unified text-to-image/image-to-image with an integrated reasoning step that plans multi-step edits and judges candidates before returning a result; up to 10 input reference images; 1K/2K/4K output; #1 across Image (Elo 1408), Graphic Design (Elo 1469), and Image Editing (Elo 1377) on DesignArena; cheaper/faster Riverflow 2.5 Fast variant also available | API credits ($0.13/image 1K – $0.17/image 4K for Pro; Fast from $0.019/image; no direct consumer site — API only) |
...
## FLUX 3 — Not Yet an Image-Generation Replacement (July 23, 2026)

Black Forest Labs launched FLUX 3 on July 23, 2026 as its first natively multimodal model — one architecture jointly trained across image, video, audio, and robot-action prediction, generating up to 20 seconds of video with synchronized native audio from text, a still image, or keyframes. As of Sept 2026, only **FLUX 3 Video** is generally available (via the BFL API and select partners); **FLUX 3 Image** (standalone text-to-image and editing) and an open-weight **FLUX 3 Dev** are still on BFL's roadmap and not yet released. Until FLUX 3 Image ships, keep recommending **FLUX.2 [max]/[pro]** (above) for static image work — FLUX 3 today is a video/audio tool, not an image-generation upgrade.
...
- Launched at **#2** on Arena.ai text-to-image, single-image edit, and multi-image edit leaderboards (Elo 1280); by Sept 2026 it's settled around #5 on the Text-to-Image Arena (Elo ~1312) as MAI-Image-2.6, Reve 2.1, and Nano Banana 2 launched or updated above it
