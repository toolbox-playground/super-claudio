...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 (became default Aug 4 2026, succeeding v8.1) — aesthetic refinements, better Sref/moodboard consistency, Big Batch Draft Mode (`--sref random`, 24× faster style exploration) now stable and on by default; v8.1 still selectable via `--v 8.1`; carries forward v8.1's 5× speed over v7, native 2K resolution, improved text rendering | Paid (no free tier) |
...
| **FLUX 3 Image** (Black Forest Labs) | bfl.ai, fal.ai, openrouter.ai | Unified multimodal flagship — same jointly-trained architecture that also generates FLUX 3 Video (20s clips with native in-sync audio, early access Jul 23 2026) and drives FLUX 3 Action robotics trials; image modality entered staged rollout Aug 2026; extends Grounded Generation with cross-modal (video/audio-aware) context and up to 10 reference images; debuts #1 on Artificial Analysis Image Generation, pushing FLUX.2 [max] to #3 (behind Nano Banana Pro) | API credits (staged rollout; per-image pricing TBD at full GA — apply via bfl.ai for early access) |
| **FLUX.2 [max]** (Black Forest Labs) | bfl.ai, fal.ai, openrouter.ai | Highest-quality prior-gen FLUX model; Grounded Generation (retrieves real-time web context during image gen); up to 10 reference images for consistent products/characters/styles; #3 on Artificial Analysis Image Generation (behind FLUX 3 Image and Nano Banana Pro); remains GA for teams not yet migrated to FLUX 3; open-source [dev] variant (32B params) released alongside | API credits ($0.07/mp first output mp, $0.03/mp subsequent; [dev]: open-weights, non-commercial) |
...
## FLUX 3 — Black Forest Labs' Multimodal Flagship (August 2026)

Black Forest Labs unveiled FLUX 3 on July 23, 2026: a single architecture jointly trained on images, video, and audio, later extended to predict robot actions (FLUX 3 Action, already being trialed on Audi production lines for soft-body manipulation tasks). FLUX 3 Video shipped first in early access, generating up to 20-second clips with native in-sync audio (text-to-video, image-to-video, video-to-video, keyframe-to-video). FLUX 3 Image — the text-to-image modality — followed in a staged rollout starting August 2026, inheriting Grounded Generation and reference-image conditioning from FLUX.2 [max] plus the cross-modal training that helps it stay consistent with the video/audio side of the family. It debuts atop the Artificial Analysis Image Generation leaderboard, pushing FLUX.2 [max] down to #3 (behind Nano Banana Pro).

- Best for: teams already on the FLUX stack wanting one model/API surface for image + video + audio; brand campaigns needing consistent assets across formats
- Access: bfl.ai, fal.ai, openrouter.ai — staged rollout; apply via Black Forest Labs for early API access
- FLUX.2 [max] / [pro] / [klein] remain generally available for teams not yet migrated
