...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; v8.2 (became default model Jul 24 2026, was in preview via `--preview` flag since Jun 25 2026) — aesthetic/quality refinements, bolder and more sophisticated default look, fewer low-quality renders, Personalization now trained on a larger image pool; supersedes v8.1 (Apr 30 2026 — 5× faster than v7, native 2K resolution, improved text rendering) | Paid (no free tier) |
...
| **Meta Muse Image** (Meta Superintelligence Labs) | meta.ai, Meta AI app, WhatsApp (limited countries) — **pulled from Instagram Jul 13, 2026** after privacy/consent backlash | Social-native image generation — text-to-image, multi-photo blending, sketch-and-instruction editing; agentic tool use (web search + coding invoked during generation); self-refinement at inference time (test-time compute scaling before delivering the final image); Content Seal invisible watermark; launched July 7, 2026; **#2 on Arena.ai Text-to-Image Arena (Elo 1280, July 2026)** — also #2 in single-image edit and multi-image edit arenas; no developer API at launch (Meta evaluating external access) | Free (consumer; subscription for power users via Meta AI subscription plans) |
...
## Meta Muse Image — Social-Native Generation (July 7, 2026; Instagram rollback Jul 13, 2026)

Meta Superintelligence Labs launched Muse Image on July 7, 2026: Meta's first image generation model from MSL. Agentic tool use means it can call web search or run code as part of a generation request. Test-time compute scaling refines the output before you see it.

**Update (Jul 13, 2026):** Meta pulled Muse Image from Instagram just days after launch. The original Instagram rollout let anyone @mention a public account to generate images using that person's likeness, opted in by default with no consent step — this drew backlash from users, privacy advocates, and SAG-AFTRA. Meta confirmed the feature "is no longer available" on Instagram. Muse Image remains available in the standalone Meta AI app and on WhatsApp; only the Instagram integration (and its 30+ Story effects) was removed.

- **Ranked #2** on Arena.ai text-to-image, single-image edit, and multi-image edit leaderboards (Elo 1280, July 2026)
- Available free at meta.ai and in the Meta AI app; WhatsApp (limited countries) — **no longer on Instagram**
...
- Best for: social-first creators already in the Meta AI app/WhatsApp ecosystem
- Not ideal for: developers needing programmatic access (no API yet); high-volume batch generation; print/4K quality output (use FLUX.2 [max] or GPT Image 2 for those); Instagram-native workflows (feature removed)
