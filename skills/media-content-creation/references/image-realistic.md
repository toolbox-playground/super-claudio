...
|------|--------|----------|
| **Midjourney v8.2** | midjourney.com | Artistic quality, concept art, portraits; **GA since July 24, 2026** (exited `--preview`, now the default for all users) — aesthetic refinements, better Sref/moodboard consistency, Big Batch Draft Mode with `--sref random` for 24× faster style exploration, on top of v8.1's 5× faster generation and native 2K resolution; pricing unchanged | Paid (no free tier; Basic $10 / Standard $30 / Pro $60 / Mega $120/mo) |
...
| **Reve 2.1** (Reve AI) | app.reve.com | Layout-first 4K generation — builds an editable code-based layout before rendering, so you can fix a misplaced headline or resize one element without re-rolling; dedicated typography step keeps headlines, packaging labels, and captions crisp and correctly spelled; text-to-image, image editing, multi-reference composition; native 4K output; **released July 9, 2026** (up from 2.0), now **#2 on Artificial Analysis Text-to-Image Arena (Elo ~1299–1306, +36 pts over 2.0)** | Free tier (daily refresh); Lite $7.99/mo; Pro $19.99/mo |
...
| **FLUX.2 [max]** (Black Forest Labs) | bfl.ai, fal.ai, openrouter.ai | Highest-quality FLUX model; Grounded Generation (retrieves real-time web context during image gen); up to 10 reference images for consistent products/characters/styles; #2 on Artificial Analysis Image Generation (behind Nano Banana Pro); open-source [dev] variant (32B params) released alongside; **watch for FLUX 3** — announced July 23, 2026 as a multimodal (image+video+audio+robot-action) successor architecture, but FLUX 3 Image has not shipped yet ("coming weeks" per BFL) and FLUX 3 Video/Action are early-access only — FLUX.2 remains BFL's current shipping image line | API credits ($0.07/mp first output mp, $0.03/mp subsequent; [dev]: open-weights, non-commercial) |
