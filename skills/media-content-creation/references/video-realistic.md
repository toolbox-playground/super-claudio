...
| **HappyHorse-1.0** (Alibaba / ATH) | Text / Image | Temporal consistency — objects & faces stay identical frame-to-frame; #1 T2V no-audio (Elo 1293), #2 T2V with-audio (Elo 1123) on Artificial Analysis (June 2026) | API credits | fal.ai |
| **SkyReels V4** (Skywork AI) | Text / Image | Native dual-stream audio+video, 1080p/32FPS, 15s clips, 93% character consistency, <120ms audio alignment; #3 Artificial Analysis Text-to-Video+Audio Arena (June 2026, Elo 1110); first open-source model with unified audio+video | Open-source (Apache 2.0); limited free via wavespeed.ai | wavespeed.ai |
