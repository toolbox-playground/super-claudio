...
| **HappyHorse-1.1** (Alibaba / ATH) | Text / Image / Multi-Ref | Upgraded motion dynamics, audio sync, and multi-reference consistency vs 1.0; 9 reference images per generation; 2,500+ char semantic input; Multi-Scene Automatic Scheduling (6–8 consecutive scenes per prompt); Multi-Character Reference; Character × Scene Free Combination; #2 T2V no-audio (Elo 1285), #2 T2V with-audio (Elo 1153), #5 I2V no-audio (Elo 1312) on Artificial Analysis (June 2026) | API credits | fal.ai, happyhorsesai.com, Alibaba Cloud |
...
- Wan 2.7 (Alibaba Cloud) released April 6, 2026 as four models: t2v, i2v, r2v (reference-to-video + voice cloning), and videoedit (instruction-based editing of existing clips); first/last frame control (FLF2V — currently the only major AI video model with this); 9-grid multi-scene generation; thinking mode for complex prompts; 1080p up to 15 seconds; Apache 2.0 open-source (27B params, MoE); API at $0.10/s (720p) or $0.15/s (1080p); also available via Together AI at $0.10/s serverless (Wan 2.7 launched on Together AI April 3, 2026 — the April Together AI launch was Wan 2.7, not Wan 3.0); limited free tier at wan.video; **Wan 3.0 (released Q2 2026 — API on Together AI since April 2026)**: 60B-param MoE diffusion transformer (14B active per pass), native 4K, up to 30-second clips, automated 2–5 minute multi-shot narratives, 6-shot AI Director mode, Identity Lock for cross-session character consistency; open weights on HuggingFace and ModelScope (Apache 2.0); API via fal.ai and Together AI
